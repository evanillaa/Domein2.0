...
local function playerConnect(source, setKickReason, deferrals)
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "Je kon niet deelnemen")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        ...
        TriggerEvent("HD-logs:server:SendLog", "joinleave", "Queue", "orange", "**".. name .. "** joining..")
    ...
    while true do
        ...
        if data.timeout >= Config.QueueTimeOut and os_time() - connectTime > 5 then
            remove("[Wachtrij] Verwijderd wegens time-out")
        ...
        if pos <= 1 and Queue:NotFull() and _Queue.JoinDelay <= GetGameTimer() then
            ...
            Queue:RemoveFromQueue(ids)
            TriggerEvent("HD-logs:server:SendLog", "joinleave", "Queue", "orange", "**".. name .. "** joining..")
            TriggerEvent("ec-log:server:sendLog", ids[1], "joinedqueue", {})
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] is loading into the server")
            return
        end
