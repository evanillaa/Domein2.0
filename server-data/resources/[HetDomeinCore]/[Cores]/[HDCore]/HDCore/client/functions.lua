HDCore.Functions = {}
HDCore.Functions.Locales = {}
HDCore.Functions.GetPlayerData = function(cb)
    if cb ~= nil then
        cb(HDCore.PlayerData)
    else
        return HDCore.PlayerData
    end
end

HDCore.Functions.GetCoords = function(entity)
...
HDCore.Functions.SpawnVehicle = function(model, cb, coords, isnetworked, IsAdmin)
    local model = (type(model)=="number" and model or GetHashKey(model))
    local coords = coords ~= nil and coords or HDCore.Functions.GetCoords(PlayerPedId())
...
HDCore.Functions.DeleteVehicle = function(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

HDCore.Functions.Notify = function(text, textype, length)
...
HDCore.Functions.TriggerCallback = function(name, cb, ...)
	HDCore.ServerCallbacks[name] = cb
    TriggerServerEvent("HDCore:Server:TriggerCallback", name, ...)
end
...
HDCore.Functions.GetStreetLabel = function()
...
HDCore.Functions.EnumerateEntities = function(initFunc, moveFunc, disposeFunc)
...
HDCore.Functions.GetVehicles = function()
    local vehicles = {}
	for vehicle in HDCore.Functions.EnumerateEntities(FindFirstVehicle, FindNextVehicle, EndFindVehicle) do
		table.insert(vehicles, vehicle)
	end
	return vehicles
end

HDCore.Functions.GetClosestVehicle = function(coords)	
	local vehicles        = HDCore.Functions.GetVehicles()
...
HDCore.Functions.GetClosestPlayer = function(coords)
	if coords == nil then
        coords = GetEntityCoords(PlayerPedId())
	end
	local closestPlayers = HDCore.Functions.GetPlayersFromCoords(coords)
    local closestDistance = -1
...
HDCore.Functions.MathRound = function(value, numDecimalPlaces)
...
HDCore.Functions.GetVehiclesInArea = function(coords, area)
	local Vehicles       = HDCore.Functions.GetVehicles()
...
HDCore.Functions.IsSpawnPointClear = function(coords, radius)
	local Vehicles = HDCore.Functions.GetVehiclesInArea(coords, radius)
	if #Vehicles == 0 then
		return true
	end
end

HDCore.Functions.MathGroupDigits = function(value)
    local left,num,right = string.match(value,'^([^%d]*%d)(%d*)(.-)$')

    return left..(num:reverse():gsub('(%d%d%d)','%1' .. _U('locale_digit_grouping_symbol')):reverse())..right
end

HDCore.Functions.MathTrim = function(value)
    if value then
...
HDCore.Functions.GetPlayersFromCoords = function(coords, distance)
...
HDCore.Functions.GetPeds = function(ignoreList)
    local ignoreList = ignoreList or {}
	local peds       = {}
	for ped in HDCore.Functions.EnumerateEntities(FindFirstPed, FindNextPed, EndFindPed) do
		local found = false
...
HDCore.Functions.GetClosestPed = function(coords, ignoreList)
	local ignoreList      = ignoreList or {}
	local peds            = HDCore.Functions.GetPeds(ignoreList)
	local closestDistance = -1
    local closestPed      = -1
    if coords == nil then
        coords = GetEntityCoords(PlayerPedId())
...
HDCore.Functions.ShowFloatingHelpNotification = function(msg, coords)
...
-- HDCore.Functions.GetVehicleProperties = function(vehicle)
...
-- 		plate             = HDCore.Functions.MathTrim(GetVehicleNumberPlateText(vehicle)),
-- 		plateIndex        = GetVehicleNumberPlateTextIndex(vehicle),
 
-- 		bodyHealth        = HDCore.Functions.MathRound(GetVehicleBodyHealth(vehicle), 1),
-- 		engineHealth      = HDCore.Functions.MathRound(GetVehicleEngineHealth(vehicle), 1),
 
-- 		fuelLevel         = HDCore.Functions.MathRound(GetVehicleFuelLevel(vehicle), 1),
-- 		dirtLevel         = HDCore.Functions.MathRound(GetVehicleDirtLevel(vehicle), 1),
...
HDCore.Functions.DrawText = function(x, y, width, height, scale, r, g, b, a, text)
	SetTextFont(4)
...
HDCore.Functions.DrawText3D = function(x, y, z, text)
...
HDCore.Functions.SetVehicleProperties = function(vehicle, props)
