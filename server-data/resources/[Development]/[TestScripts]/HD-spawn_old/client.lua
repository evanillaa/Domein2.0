...
RegisterNetEvent('HD-spawn:client:openUI')
AddEventHandler('HD-spawn:client:openUI', function(value, new)
...
RegisterNUICallback('chooseAppa', function(data)
    ...
    TriggerServerEvent('HDCore:Server:OnPlayerLoaded')
    TriggerEvent('HDCore:Client:OnPlayerLoaded')
...
RegisterNUICallback('spawnplayer', function(data)
    ...
    if type == "current" then
        ...
        if insideMeta.house ~= nil then
            ...
            TriggerEvent('HD-houses:client:LastLocationHouse', houseId)
        elseif insideMeta.apartment.apartmentType ~= nil or insideMeta.apartment.apartmentId ~= nil then
            local apartmentType = insideMeta.apartment.apartmentType
            local apartmentId = insideMeta.apartment.apartmentId
            TriggerEvent('HD-apartments:client:LastLocationHouse', apartmentType, apartmentId)
        end
        TriggerServerEvent('HDCore:Server:OnPlayerLoaded')
        TriggerEvent('HDCore:Client:OnPlayerLoaded')
        FreezeEntityPosition(ped, false)
    ...
    elseif type == "house" then
        ...
        DoScreenFadeOut(500)
        Citizen.Wait(2000)
        TriggerEvent('HD-houses:client:enterOwnedHouse', location)
        TriggerServerEvent('HDCore:Server:OnPlayerLoaded')
        TriggerEvent('HDCore:Client:OnPlayerLoaded')
        TriggerServerEvent('HD-houses:server:SetInsideMeta', 0, false)
        TriggerServerEvent('HD-apartments:server:SetInsideMeta', 0, 0, false)
        FreezeEntityPosition(ped, false)
        RenderScriptCams(false, true, 500, true, true)
        SetCamActive(cam, false)
        DestroyCam(cam, true)
...
RegisterNetEvent('HD-houses:client:setHouseConfig')
AddEventHandler('HD-houses:client:setHouseConfig', function(houseConfig)
    Config.Houses = houseConfig
end)

RegisterNetEvent('HD-spawn:client:setupSpawns')
AddEventHandler('HD-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        HDCore.Functions.TriggerCallback('HD-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
