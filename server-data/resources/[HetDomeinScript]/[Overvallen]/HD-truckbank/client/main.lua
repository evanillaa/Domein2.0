        ...
        if CurrentCops >= Config.PoliceNeeded then
          if not Config.RobbedPlates[Plate] then
            ...
            HDCore.Functions.Notify("Niet genoeg politie.. (4 nodig)", "error")
              ...
              HDCore.Functions.Notify("Dit is geen geld wagen nerd..", "error")
          end
      else
...
function StartRobbingVan(Veh)
    ...
    SearchAnim()
    HDCore.Functions.Progressbar("open_locker", "Bank pas gebruiken..", math.random(25000, 50000), false, true, {
    ...
    Citizen.CreateThread(function()
        while IsRobbing do
            TriggerServerEvent('HD-hud:Server:gain:stress', math.random(2, 3))
            Citizen.Wait(12000)
        end
    end)
end
