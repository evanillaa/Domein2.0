            ...
            for k, v in pairs(Config.SpecialBanks) do
                    ...
                    for Troll, Trolly in pairs(Config.Trollys) do
                        ...
                        if Distance < 1.5 then
                            ...
                            if not Trolly['Open-State'] then
                             ...
                             HDCore.Functions.DrawText3D(Trolly['Grab-Coords']['X'], Trolly['Grab-Coords']['Y'], Trolly['Grab-Coords']['Z'],_U("grabmoney"))
                   ...
                   if not v['Open'] then
                        ...
                        if Area < 1.3 then
                            ...
                            TriggerEvent('HD-inventory:client:requiredItems', PacificItems, true)
                            end
                        end
                   end
                end
    ...
    while true do
        ...
        if LoggedIn then
            if GrabbingMoney then
              TriggerServerEvent('HD-bankrobbery:server:rob:pacific:money')
              Citizen.Wait(1000)
            end
        end
    end
...
RegisterNetEvent('HD-bankrobbery:client:use:black-card')
AddEventHandler('HD-bankrobbery:client:use:black-card', function()
    for k, v in pairs(Config.SpecialBanks) do 
            if not v['Open'] then
                local Area = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), v['Hack-Door']['X'], v['Hack-Door']['Y'], v['Hack-Door']['Z'], true)
                if Area < 1.35 then
                        ...
                        HDCore.Functions.TriggerCallback('HD-bankrobbery:server:HasItem', function(HasItem)
                            if HasItem then
                                HDCore.Functions.TriggerCallback("HD-bankrobbery:server:get:status", function(Status)
                                    if not Status then
                                        TriggerServerEvent('HD-police:server:send:big:bank:alert', GetEntityCoords(PlayerPedId()), HDCore.Functions.GetStreetLabel())
                                        TriggerEvent('HD-inventory:client:set:busy', true)
                                        TriggerEvent('HD-inventory:client:requiredItems', PacificItems, false)
                                        ...
                                        exports['minigame-phone']:StartHack(math.random(1,4), 130, function(Success)
                                            if Success then
                                                TriggerEvent("utk_fingerprint:Start", 2, 2, 2, function(Outcome)
                                                    if Outcome then
                                                        CreateTrollys()
                                                        TriggerServerEvent('HD-bankrobbery:server:pacific:start')
                                                        TriggerServerEvent('HDCore:Server:RemoveItem', 'black-card', 1)
                                                        TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items['black-card'], "remove")
                                        ...
                                        HDCore.Functions.Notify(_U("alreadystarted"), "error")
                                    end
                                end)
                            else
...
RegisterNetEvent('HD-bankrobbery:client:pacific:start')
AddEventHandler('HD-bankrobbery:client:pacific:start', function()
    OpenPacificDoor()
    Config.SpecialBanks[1]['Open'] = true
end)

RegisterNetEvent('HD-bankrobbery:client:clear:trollys')
AddEventHandler('HD-bankrobbery:client:clear:trollys', function()
    Count = 0
...
RegisterNetEvent('HD-bankrobbery:client:set:trolly:state')
AddEventHandler('HD-bankrobbery:client:set:trolly:state', function(TrollyNumber, bool)
...
function GetMoneyFromTrolly(TrollyNumber)
    ...
    GrabbingMoney = false
    TriggerServerEvent('HD-bankrobbery:server:set:trolly:state', TrollyNumber, true)
