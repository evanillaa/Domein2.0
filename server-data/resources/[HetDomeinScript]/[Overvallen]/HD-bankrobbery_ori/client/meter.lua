...
function lockpickDone(success)
    ...
    if success then
        ...
        if math.random(1, 100) <= 65 and not IsWearingHandshoes() then
            TriggerServerEvent("HD-police:server:CreateFingerDrop", GetEntityCoords(PlayerPedId()))
            HDCore.Functions.Notify("Handschoenen zijn gescheurd..")
        end
...
function PoliceCall()
    ...
    if math.random(1,100) < 40 then
        ...
        TriggerServerEvent('HD-police:server:send:alert:meter', GetEntityCoords(GetPlayerPed(-1)), StreetLabel)
     end
end


RegisterNetEvent('HD-parkmeter:client:open')
AddEventHandler('HD-parkmeter:client:open', function()
            ...
            if canRob == true then
                ...
                exports['HD-lockpick']:OpenLockpickGame(function(Success)
                    if Success then
                        ...
                        HDCore.Functions.Progressbar("search_parkmeter", "Parkeermeter beroven...", 12000, false, true, {
                        ...
                        HDCore.Functions.Notify("Heloas")
                       end)
                    end
                end)
            else
                HDCore.Functions.Notify('Cooldown. Over '..timer..' seconden kan je het weer proberen', 'error')
            end
        else
            HDCore.Functions.Notify('Je hebt niet de benodigde items!', 'error')
        end
    end, "lockpick")
end)
...
function LockPickRegisterAnim(time)
    ...
    Citizen.CreateThread(function()
        while canRob do
            TriggerServerEvent('HD-hud:server:gain:stress', 1) 
