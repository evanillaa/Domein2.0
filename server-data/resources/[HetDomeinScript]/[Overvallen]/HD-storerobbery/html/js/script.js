...
StoreRob.Functions.ClosePinpad = function(HasPutin) {
    ...
    $.post('http://HD-storerobbery/PinpadClose', JSON.stringify({
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        ...
        case 13:
            var v = $("#PINbox").val();
            if (v == "") {
                $.post('http://HD-storerobbery/ClickFail', JSON.stringify({}))
                $.post('http://HD-storerobbery/ErrorMessage', JSON.stringify({
                ...
                $("#PINbox").val("");
                $.post('http://HD-storerobbery/Click', JSON.stringify({}))
                $.post('http://HD-storerobbery/EnterPincode', JSON.stringify({
                    pin: data.pin
                }))
