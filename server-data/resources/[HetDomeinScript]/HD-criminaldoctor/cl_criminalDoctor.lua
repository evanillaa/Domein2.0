  ...
  while true do 
      ...
      for k,v in ipairs(Config.Doctor) do
          if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 2.0) then     
              Draw3DText(v.x, v.y, v.z, "~b~[E]~s~ om geholpen te worden [~g~$" .. Config.toPay .. "~s~]" )
              ...
              elseif IsControlJustReleased(0, 38) and IsEntityDead(playerPed) and CurrentBed ~= nil then
                ...
                HDCore.Functions.Notify("Je bent niet gewond", "error")
              end
            -- end)
          end
      end
...
HDCore.Functions.Progressbar("use_bandage", "You are being treated", 60000, false, true, {
