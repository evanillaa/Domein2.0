...
RegisterNetEvent('HD-fight:playerJoined')
AddEventHandler('HD-fight:playerJoined', function(side, id)
...
RegisterNetEvent('HD-fight:startFight')
AddEventHandler('HD-fight:startFight', function(fightData)
...
RegisterNetEvent('HD-fight:playerLeaveFight')
AddEventHandler('HD-fight:playerLeaveFight', function(id)

    if id == GetPlayerServerId(PlayerId()) then
		HDCore.Functions.Notify(_U("toofar"), "error")

        SetPedMaxHealth(PlayerPedId(), 200)
        SetEntityHealth(PlayerPedId(), 200)
        removeGloves()
    elseif participando == true then
        TriggerServerEvent('HD-fight:pay', betAmount)	
		HDCore.Functions.Notify(_U("winmsg") .."' .. (betAmount * 2) .. '€", "error")
...
RegisterNetEvent('HD-fight:fightFinished')
AddEventHandler('HD-fight:fightFinished', function(looser)
    ...
    if participando == true then
        if(looser ~= GetPlayerServerId(PlayerId()) and looser ~= -2) then
            ...
            TriggerServerEvent('HD-fight:showWinner', GetPlayerServerId(PlayerId()))
        end
    
        if(looser == GetPlayerServerId(PlayerId()) and looser ~= -2) then
		
			HDCore.Functions.Notify(_U("lostmsg") .. "' .. betAmount .. '€", "error")
            SetPedMaxHealth(PlayerPedId(), 200)
            SetEntityHealth(PlayerPedId(), 200)
        end
    
        if looser == -2 then
			HDCore.Functions.Notify(_U("timelimit"), "error")
            SetPedMaxHealth(PlayerPedId(), 200)
            SetEntityHealth(PlayerPedId(), 200)
        end
...
RegisterNetEvent('HD-fight:raiseActualBet')
AddEventHandler('HD-fight:raiseActualBet', function()
...
RegisterNetEvent('HD-fight:winnerText')
AddEventHandler('HD-fight:winnerText', function(id)
    showWinner = true
    winner = id
    Citizen.Wait(5000)
    showWinner = false
...
function spawnMarker(coords)
    ...
    if centerRing < Config.DISTANCE and fightStatus ~= STATUS_STARTED then
       ...
       HDCore.Functions.DrawText3D(Config.CENTER.x, Config.CENTER.y, Config.CENTER.z +1.5, 'Fighters: ~r~' .. players .. '/2 \n ~w~Bet: ~r~'.. betAmount ..'$ ', 0.4)
        ...
        if blueJoined == false then
            HDCore.Functions.DrawText3D(Config.BLUEZONE.x, Config.BLUEZONE.y, Config.BLUEZONE.z +1.5, _U("participate"), 0.4)
            if blueZone < Config.DISTANCE_INTERACTION then
               if IsControlJustReleased(0, Config.E_KEY) and participando == false then
                    TriggerServerEvent('HD-fight:join', betAmount, 0 )
                end
            end
        end

        if redJoined == false then
            HDCore.Functions.DrawText3D(Config.REDZONE.x, Config.REDZONE.y, Config.REDZONE.z +1.5, _U("participate"), 0.4)
            if redZone < Config.DISTANCE_INTERACTION then
                if IsControlJustReleased(0, Config.E_KEY) and participando == false then
                    TriggerServerEvent('HD-fight:join', betAmount, 1)
                end
            end
        end
    end
    ...
    while true do
        if fightStatus == STATUS_STARTED and participando == false and GetEntityCoords(PlayerPedId()) ~= rival then
            ...
            if get3DDistance(Config.CENTER.x, Config.CENTER.y, Config.CENTER.z,coords.x,coords.y,coords.z) < Config.TP_DISTANCE then
				HDCore.Functions.Notify(_U("stayaway"), "error")
    ...
    while true do
        ...
        elseif showCountDown == false and fightStatus == STATUS_STARTED then
            if GetEntityHealth(PlayerPedId()) < 150 then
                TriggerServerEvent('HD-fight:finishFight', GetPlayerServerId(PlayerId()))
                fightStatus = STATUS_INITIAL
            end
        end
        ...
        if participando == true then
            ...
            if get3DDistance(Config.CENTER.x, Config.CENTER.y, Config.CENTER.z,coords.x,coords.y,coords.z) > Config.LEAVE_FIGHT_DISTANCE then
                TriggerServerEvent('HD-fight:leaveFight', GetPlayerServerId(PlayerId()))
            end
        end
        ...
        if showWinner == true and winner ~= nil then
            ...
            if get3DDistance(Config.CENTER.x, Config.CENTER.y, Config.CENTER.z,coords.x,coords.y,coords.z) < 5 then
                HDCore.Functions.DrawText3D(Config.CENTER.x, Config.CENTER.y, Config.CENTER.z + 2.5, '~r~Deelnemer: ' .. winner .. ' winnaar!', 2.0)
            end
        end
    end
end)