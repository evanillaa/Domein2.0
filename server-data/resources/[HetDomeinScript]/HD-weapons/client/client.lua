               ...
               if Config.WeaponsList[Weapon]['IdName'] ~= 'weapon_unarmed' then 
                if IsPedShooting(GetPlayerPed(-1)) or IsPedPerformingMeleeAction(GetPlayerPed(-1)) then
                    if Config.WeaponsList[Weapon]['IdName'] == 'weapon_molotov' then
                        TriggerServerEvent('HDCore:Server:RemoveItem', 'weapon_molotov', 1)
                        TriggerEvent('HD-weapons:client:set:current:weapon', nil)
                        TriggerEvent('HD-inventory:client:ItemBox', HDCore.Shared.Items['weapon_molotov'], 'remove')
                    else
                        TriggerServerEvent("HD-weapons:server:UpdateWeaponQuality", Config.CurrentWeaponData, 1)
                        if WeaponBullets == 1 then
                          TriggerServerEvent("HD-weapons:server:UpdateWeaponAmmo", Config.CurrentWeaponData, 1)
                        else
                          TriggerServerEvent("HD-weapons:server:UpdateWeaponAmmo", Config.CurrentWeaponData, tonumber(WeaponBullets))
                        end
                    end
                end
...
AddEventHandler('HD-weapons:client:set:current:weapon', function(data)
    if data ~= false then
...
RegisterNetEvent('HD-weapons:client:set:quality')
AddEventHandler('HD-weapons:client:set:quality', function(amount)
    if Config.CurrentWeaponData ~= nil and next(Config.CurrentWeaponData) ~= nil then
        TriggerServerEvent("HD-weapons:server:SetWeaponQuality", Config.CurrentWeaponData, amount)
    end
end)

RegisterNetEvent("HD-weapons:client:EquipAttachment")
AddEventHandler("HD-weapons:client:EquipAttachment", function(ItemData, attachment)
    local weapon = GetSelectedPedWeapon(GetPlayerPed(-1))
    ...
    if weapon ~= GetHashKey("WEAPON_UNARMED") then
        ...
        if Config.WeaponAttachments[WeaponData['IdName']] ~= nil then
            if Config.WeaponAttachments[WeaponData['IdName']][attachment] ~= nil then
                TriggerServerEvent("HD-weapons:server:EquipAttachment", ItemData, Config.CurrentWeaponData, Config.WeaponAttachments[WeaponData['IdName']][attachment])
            else
                HDCore.Functions.Notify("The weapon you are using does not support these attachments.", "error")
            end
        end
    else
        HDCore.Functions.Notify("Grab a weapon.", "error")
    end
end)

RegisterNetEvent('HD-weapons:client:reload:ammo')
AddEventHandler('HD-weapons:client:reload:ammo', function(AmmoType, AmmoName)
 local Weapon = GetSelectedPedWeapon(GetPlayerPed(-1))
 local WeaponBullets = GetAmmoInPedWeapon(GetPlayerPed(-1), Weapon)
 if Config.WeaponsList[Weapon] ~= nil and Config.WeaponsList[Weapon]['AmmoType'] ~= nil then
 ...
 if Config.WeaponsList[Weapon]['AmmoType'] == AmmoType then
    if WeaponBullets <= 1 then
	reloading = true
    TriggerServerEvent('HDCore:Server:RemoveItem', AmmoName, 1)
    TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items[AmmoName], "remove")
        HDCore.Functions.Progressbar("taking_bullets", "Herladen...", math.random(2000, 6000), false, true, {
            ...
            disableCombat = true,
             ...
             TriggerServerEvent("HD-weapons:server:UpdateWeaponAmmo", Config.CurrentWeaponData, tonumber(NewAmmo))
             HDCore.Functions.Notify("+ "..NewAmmo.."x bullets ("..Config.WeaponsList[Weapon]['Name']..")", "success")
             reloading = false
        end, function()
            HDCore.Functions.Notify("Canceled.", "error")
            
            TriggerServerEvent('HDCore:Server:AddItem', AmmoName, 1)
            TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items[AmmoName], "add")
				reloading = false
        end)
    else
        HDCore.Functions.Notify("Weapon is already loaded.", "error")
			reloading = false
    end
  end
 end
...
RegisterNetEvent('HD-weapons:client:set:ammo')
AddEventHandler('HD-weapons:client:set:ammo', function(Amount)
 local Weapon = GetSelectedPedWeapon(GetPlayerPed(-1))
 local WeaponBullets = GetAmmoInPedWeapon(GetPlayerPed(-1), Weapon)
 ...
 if Config.WeaponsList[Weapon] ~= nil and Config.WeaponsList[Weapon]['AmmoType'] ~= nil then
  ...
  TriggerServerEvent("HD-weapons:server:UpdateWeaponAmmo", Config.CurrentWeaponData, tonumber(NewAmmo))
  HDCore.Functions.Notify("Recieved "..Amount..'x bullets ('..Config.WeaponsList[Weapon]['Name']..')', "success", 4500)
 end
end)

RegisterNetEvent('HD-weapons:client:remove:dot')
AddEventHandler('HD-weapons:client:remove:dot', function()
...
RegisterNetEvent("HD-weapons:client:addAttachment")
AddEventHandler("HD-weapons:client:addAttachment", function(component)
 local weapon = GetSelectedPedWeapon(GetPlayerPed(-1))
 local WeaponData = Config.WeaponsList[weapon]
