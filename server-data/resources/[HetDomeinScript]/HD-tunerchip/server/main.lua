...
HDCore.Functions.CreateUseableItem("tunerlaptop", function(source, item)
    local src = source

    TriggerClientEvent('HD-tunerchip:client:openChip', src)
end)

RegisterServerEvent('HD-tunerchip:server:TuneStatus')
AddEventHandler('HD-tunerchip:server:TuneStatus', function(plate, bool)
    if bool then
...
HDCore.Functions.CreateCallback('HD-tunerchip:server:HasChip', function(source, cb)
    local src = source
...
HDCore.Functions.CreateCallback('HD-tunerchip:server:GetStatus', function(source, cb, plate)
    cb(tunedVehicles[plate])
end)

HDCore.Functions.CreateUseableItem("nitrous", function(source, item)
