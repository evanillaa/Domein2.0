...
function IsVehicleLoaded(veh)
    return false
end

RegisterNUICallback('save', function(data)
    HDCore.Functions.TriggerCallback('HD-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = GetPlayerPed(-1)
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            HDCore.Functions.Notify('Tuning saved!', 'error')

            TriggerServerEvent('HD-tunerchip:server:TuneStatus', GetVehicleNumberPlateText(veh), true)
        end
    end)
end)

RegisterNetEvent('HD-tunerchip:server:TuneStatus')
AddEventHandler('HD-tunerchip:server:TuneStatus', function()
    local ped = GetPlayerPed(-1)
    ...
    HDCore.Functions.TriggerCallback('HD-tunerchip:server:GetStatus', function(status)
        if status then
            TriggerEvent("chatMessage", "VOERTUIG STATUS", "warning", displayName..": Chiptuned: Yes")
        else
            TriggerEvent("chatMessage", "VOERTUIG STATUS", "warning", displayName..": Chiptuned: No")
        end
    end, plate)
end)
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    HDCore.Functions.Notify('Tuner Tablet: Reset!', 'error')
end)

RegisterNetEvent('HD-tunerchip:client:openChip')
AddEventHandler('HD-tunerchip:client:openChip', function()
    local ped = GetPlayerPed(-1)
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        HDCore.Functions.Progressbar("connect_laptop", "Connecting Tuner Tablet...", 2000, false, true, {
        ...
        HDCore.Functions.Notify("You are not in a vehicle.", "error")
    end
end)
