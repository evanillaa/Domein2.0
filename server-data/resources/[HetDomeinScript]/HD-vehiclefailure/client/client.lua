...
RegisterNetEvent('HD-vehiclefailure:client:fix:veh')
AddEventHandler('HD-vehiclefailure:client:fix:veh', function()
	if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
		...
		SetVehicleFuelLevel(Vehicle, Amount + 0.0)
 		TriggerServerEvent('HD-fuel:server:update:fuel', vehiclePlate, Vehicle, math.floor(Amount))
	else
		local Vehicle = HDCore.Functions.GetClosestVehicle()
		SetVehicleFixed(Vehicle)
	end
	HDCore.Functions.Notify("Voertuig is gerepareerd", 'success')
end)