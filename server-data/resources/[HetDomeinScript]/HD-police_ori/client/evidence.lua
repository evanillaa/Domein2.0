			...
			if Weapon ~= GetHashKey("WEAPON_UNARMED") and Weapon ~= GetHashKey("WEAPON_SNOWBALL") and Weapon ~= GetHashKey("WEAPON_STUNGUN") and Weapon ~= GetHashKey("WEAPON_PETROLCAN") and Weapon ~= GetHashKey("WEAPON_FIREEXTINGUISHER") and Weapon ~= GetHashKey("WEAPON_MOLOTOV") then
				...
				if ShotsFired > 5 and (CurrentStatusList == nil or CurrentStatusList["gunpowder"] == nil) then
					if math.random(1, 10) <= 7 then
						TriggerEvent("HD-police:client:SetStatus", "gunpowder", 200)
					end
				end
...
Citizen.CreateThread(function()
	while true do
		...
		if isLoggedIn then
		if IsPedShooting(PlayerPedId()) then
		 if PlayerJob.name ~= "police" and onDuty or PlayerJob.name == "police" and not onDuty then
			local Weapon = GetSelectedPedWeapon(PlayerPedId())
			local SilentWeapon = IsWeaponSilent(Weapon)
			local GunCategory = GetWeaponCategory(Weapon)
			if not SilentWeapon and not GunShotAlert then
				ShotsFiredAlert = ShotsFiredAlert + 1
				if ShotsFiredAlert == 2 then
					GunShotAlert = true
					ShotsFiredAlert = 0
					TriggerServerEvent('HD-hud:server:gain:stress', math.random(1, 3))
					TriggerServerEvent('HD-police:server:send:alert:gunshots', GetEntityCoords(PlayerPedId()), GunCategory, HDCore.Functions.GetStreetLabel(), IsPedInAnyVehicle(PlayerPedId()))
				end
	...
	while true do
		...
		if isLoggedIn then
			if CurrentStatusList ~= nil and next(CurrentStatusList) ~= nil then
				...
				TriggerServerEvent("HD-police:server:UpdateStatus", CurrentStatusList)
			end
		...
		if CurrentCasing ~= nil then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, true) < 1.5 then
				...
				if IsControlJustReleased(0, Config.Keys["G"]) then
					...
					local AmmoType = exports['HD-weapons']:GetAmmoType(Casings[CurrentCasing].type)
					...
					TriggerServerEvent("HD-police:server:AddEvidenceToInventory", 'casing', CurrentCasing, info)
				end
			end
		end
		...
		if CurrentBlooddrop ~= nil then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y, Blooddrops[CurrentBlooddrop].coords.z, true) < 1.5 then
				...
				if IsControlJustReleased(0, Config.Keys["G"]) then
					...
					TriggerServerEvent("HD-police:server:AddEvidenceToInventory", 'blood', CurrentBlooddrop, info)
				end
			end
		end
		...
		if CurrentFingerprint ~= nil then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y, Fingerprints[CurrentFingerprint].coords.z, true) < 1.5 then
				...
				if IsControlJustReleased(0, Config.Keys["G"]) then
					...
					TriggerServerEvent("HD-police:server:AddEvidenceToInventory", 'finger', CurrentFingerprint, info)
				end
			end
		end
		...
		if CurrentHair ~= nil then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Hairs[CurrentHair].coords.x, Hairs[CurrentHair].coords.y, Hairs[CurrentHair].coords.z, true) < 1.5 then
				...
				if IsControlJustReleased(0, Config.Keys["G"]) then
					-- PickUp Event
					local StreetLabel = HDCore.Functions.GetStreetLabel()
					local info = {label = "Haarplukje", type = "hair", street = StreetLabel:gsub("%'", ""), hair = Hairs[CurrentHair].hair}
					TriggerServerEvent("HD-police:server:AddEvidenceToInventory", 'hair', CurrentHair, info)
				end
			end
		end
		...
		if CurrentSlimeDrop ~= nil then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, SlimeDrops[CurrentSlimeDrop].coords.x, SlimeDrops[CurrentSlimeDrop].coords.y, SlimeDrops[CurrentSlimeDrop].coords.z, true) < 1.5 then
				...
				if IsControlJustReleased(0, Config.Keys["G"]) then
					...
					TriggerServerEvent("HD-police:server:AddEvidenceToInventory", 'slime', CurrentSlimeDrop, info)
				end
			end
		end
	else
...
RegisterNetEvent('HD-police:client:AddBlooddrop')
AddEventHandler('HD-police:client:AddBlooddrop', function(bloodId, bloodtype, coords)
    Blooddrops[bloodId] = {
		bloodtype = bloodtype,
		coords = {
			x = coords.x,
...
RegisterNetEvent('HD-police:client:AddFingerPrint')
AddEventHandler('HD-police:client:AddFingerPrint', function(fingerId, fingerprint, coords)
...
RegisterNetEvent('HD-police:client:AddHair')
AddEventHandler('HD-police:client:AddHair', function(hairId, hair, coords)
    Hairs[hairId] = {
		hair = hair,
		coords = {
			x = coords.x,
...
RegisterNetEvent('HD-police:client:AddSlime')
AddEventHandler('HD-police:client:AddSlime', function(slimeId, slime, coords)
    SlimeDrops[slimeId] = {
		slime = slime,
		coords = {
			x = coords.x,
...
RegisterNetEvent('HD-police:client:AddCasing')
AddEventHandler('HD-police:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
		type = weapon,
		serie = serie ~= nil and serie or "Serienummer niet zichtbaar...",
		coords = {
			x = coords.x,
			y = coords.y, 
			z = coords.z - 0.9,
...
RegisterNetEvent("HD-police:client:RemoveDnaId")
AddEventHandler("HD-police:client:RemoveDnaId", function(Type, DnaId)
	if Type == 'casing' then
	 Casings[DnaId] = nil
...
RegisterNetEvent('HD-police:client:SetStatus')
AddEventHandler('HD-police:client:SetStatus', function(statusId, time)
...
function DropBulletCasing(Weapon)
 ...
 TriggerServerEvent("HD-police:server:CreateCasing", Weapon, Coords)
 Citizen.Wait(300)
end
