    ...
    while true do
        ...
        if LoggedIn then
            ...
            if Distance <= 2.0 then
                ...
                if not IsSelling then
                  ...
                  if IsControlJustReleased(0, 38) then
                      IsSelling = true
                      HDCore.Functions.Notify('Selling..', 'info')
                      TriggerServerEvent('HD-fishing:server:sell:items')
            ...
            if Distance <= 2.0 then
                ...
                if IsControlJustReleased(0, 38) then
                    HDCore.Functions.TriggerCallback("HD-fishing:server:can:pay", function(DidPay)
                        if DidPay then
                            SpawnFishBoat()
                        end
...
RegisterNetEvent('HD-fishing:client:rod:anim')
AddEventHandler('HD-fishing:client:rod:anim', function()
    exports['HD-assets']:AddProp('FishingRod')
    exports['HD-assets']:RequestAnimationDict('amb@world_human_stand_fishing@idle_a')
    TaskPlayAnim(GetPlayerPed(-1), "amb@world_human_stand_fishing@idle_a", "idle_a", 3.0, 3.0, -1, 49, 0, 0, 0, 0)
end)

RegisterNetEvent('HD-fishing:client:use:fishingrod')
AddEventHandler('HD-fishing:client:use:fishingrod', function()
       ...
       if Config.CanFish then
          if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
           if not IsEntityInWater(GetPlayerPed(-1)) then
               ...
               TriggerEvent('HD-fishing:client:rod:anim')
               Skillbar.Start({
                   ...
                   if SucceededAttempts + 1 >= NeededAttempts then
                       ...
                       SucceededAttempts = 0
                       TriggerServerEvent('HD-fishing:server:fish:reward')
                   ...
                   exports['HD-assets']:RemoveProp()
                   Config.UsingRod = false
                   HDCore.Functions.Notify('You failed..', 'error')
                   SucceededAttempts = 0
                   StopAnimTask(GetPlayerPed(-1), "amb@world_human_stand_fishing@idle_a", "idle_a", 1.0)
               end)
           else
           ...
           HDCore.Functions.Notify('You are not in the fishing area.', 'error')
       end
      end
  end)
end)
...
function SpawnFishBoat()
    ...
    HDCore.Functions.SpawnVehicle('dinghy', function(vehicle)
     ...
     HDCore.Functions.Notify('You have received your boat', 'success')
