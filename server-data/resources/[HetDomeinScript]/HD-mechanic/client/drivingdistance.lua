                ...
                if seat == ped then
                    if not CheckDone then
                        if vehiclemeters == -1 then
                            CheckDone = true
                            HDCore.Functions.TriggerCallback('HD-mechanic:server:IsVehicleOwned', function(IsOwned)
                                if IsOwned then
                ...
                if vehiclemeters ~= -1 then
                    if seat == ped then
                        if previousvehiclepos ~= nil then
                            ...
                            if DamageKey ~= nil then
                                ...
                                if chance == odd then
                                    for k, v in pairs(Config.Damages) do
                                        ...
                                        HDCore.Functions.TriggerCallback('HD-mechanic:server:SetPartLevel', function(result)
                                        end, plate, k, newDamage)
                                        -- TriggerServerEvent('HD-mechanic:server:SetPartLevel', plate, k, newDamage)
                                    end
                                end
                            end
                            ...
                            TriggerEvent('HD-hud:client:UpdateDrivingMeters', true, amount)
                            HDCore.Functions.TriggerCallback('HD-mechanic:server:UpdateDrivingDistance', function(result)
                            end, DrivingDistance[plate], plate)
                            -- TriggerServerEvent('HD-mechanic:server:UpdateDrivingDistance', DrivingDistance[plate], plate)
                        end
                    else
                        if invehicle then
                            if DrivingDistance[plate] ~= nil then
...
RegisterNetEvent('HD-mechanic:client:UpdateDrivingDistance')
AddEventHandler('HD-mechanic:client:UpdateDrivingDistance', function(amount, plate)
