    ...
    while true do
        ...
        if (PlayerJob.name ~= "police") then
            if IsPlayerFreeAiming(PlayerId()) and not IsPedShooting(PlayerPedId()) then
                ...
                local WeaponData = HDCore.Shared.Weapons[CurrentWeapon]
    ...
    while true do
        if not IsGaining then
            ...
            if StressGain > 0 then
                HDCore.Functions.Notify('Stress Gekregen', "primary", 2000)
                TriggerServerEvent('HD-hud:Server:UpdateStress', StressGain)
                StressGain = 0
            end
        end
