        ...
        if LoggedIn then
            if InTrunk then
                if IsEntityDead(GetPlayerPed(-1)) then
                    ...
                    TriggerServerEvent('HD-eye:server:set:trunk:data', Plate, false)
                    InTrunk = false
                end
                Citizen.Wait(150)
            else
...
function SendCurrentObjectData()
    if Config.ObjectOptions[CurrentEntity['Hash']] ~= nil then
        if Config.ObjectOptions[CurrentEntity['Hash']]['Job'] ~= nil then
            if HDCore.Functions.GetPlayerData().job.name == Config.ObjectOptions[CurrentEntity['Hash']]['Job'] then
                if Config.ObjectOptions[CurrentEntity['Hash']]['UseDuty'] then
                    if HDCore.Functions.GetPlayerData().job.onduty then
        ...
        elseif Config.ObjectOptions[CurrentEntity['Hash']]['MetaData'] ~= nil then
            if Config.ObjectOptions[CurrentEntity['Hash']]['MetaData'] == 'jail' then
                if HDCore.Functions.GetPlayerData().metadata['jail'].Injail then
                    if not CurrentEntity['IsPlayer'] then
                        SendNUIMessage({
                            action = 'SetData',
                            specific = false,
                ...
                if HDCore.Functions.GetPlayerData().metadata[Config.ObjectOptions[CurrentEntity['Hash']]['MetaData']] then
                    if not CurrentEntity['IsPlayer'] then
                        SendNUIMessage({
                            action = 'SetData',
                            specific = false,
            ...
            if CurrentEntity['Hash'] == GetHashKey('v_ind_bin_01') then
                ...
                if ObjectData[1]['Job'] == HDCore.Functions.GetPlayerData().job.name then
                    if HDCore.Functions.GetPlayerData().job.onduty then
                        SendNUIMessage({
                            action = 'SetData',
                            specific = false,
    ...
    elseif CurrentEntity['IsCar'] then
        if HDCore.Functions.GetPlayerData().job.name == 'cardealer' then
            SendNUIMessage({
                action = 'SetData',
                specific = false,
...
RegisterNetEvent('HD-eye:client:open:trunk')
AddEventHandler('HD-eye:client:open:trunk', function()
...
RegisterNetEvent('HD-eye:client:sync:trunk:data')
AddEventHandler('HD-eye:client:sync:trunk:data', function(ConfigData)
    Config.TrunkData = ConfigData
end)

RegisterNetEvent('HD-eye:client:getin:trunk')
AddEventHandler('HD-eye:client:getin:trunk', function()
    ...
    if Vehicle ~= 0 and Distance < 5.0 then
        ...
        if Config.TrunkData[Plate] == nil then
            TriggerServerEvent('HD-eye:server:setup:trunk:data', Plate)
            Citizen.Wait(150)
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
                    ...
                    if ClassData['CanEnter'] then
                        InTrunk = true
                        exports['HD-assets']:RequestAnimationDict("fin_ext_p1-7")
                        TriggerServerEvent('HD-eye:server:set:trunk:data', Plate, true)
                        ...
                        HDCore.Functions.Notify('Zelf ooit in een kofferbak van een motorfiets gehopt?', 'error', 2500)
                    end
                else
                    HDCore.Functions.Notify('Is de kofferbak open?', 'error', 2500)
                end
            else
                HDCore.Functions.Notify('Er zit al iemand in de kofferbak.', 'error', 2500)
            end
        else
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
        ...
        HDCore.Functions.Notify('Geen voertuig gevonden.', 'error', 2500)
    end
end)

RegisterNetEvent('HD-eye:client:getout:trunk')
AddEventHandler('HD-eye:client:getout:trunk', function()
    local Vehicle = GetEntityAttachedTo(PlayerPedId())
    local VehicleCoords = GetOffsetFromEntityInWorldCoords(Vehicle, 0, -5.0, 0)
    local Plate = GetVehicleNumberPlateText(Vehicle)
    if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
...
RegisterNetEvent('HD-eye:client:refresh')
AddEventHandler('HD-eye:client:refresh', function()
    HDCore.Functions.Progressbar("reset-eye", "Ui wordt gereset..", 1200, false, true, {
