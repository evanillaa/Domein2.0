...
HDCore.Functions.CreateCallback("HD-garage:server:is:vehicle:owner", function(source, cb, plate)
    HDCore.Functions.ExecuteSql(true, "SELECT * FROM `characters_vehicles` WHERE `plate` = '"..PlateEscapeSqli(plate).."'", function(result)
...
HDCore.Functions.CreateCallback("HD-garage:server:GetHouseVehicles", function(source, cb, HouseId)
  HDCore.Functions.ExecuteSql(true, "SELECT * FROM `characters_vehicles` WHERE `garage` = '"..HouseId.."'", function(result)
    if result ~= nil then
      cb(result)
    end 
  end)
...
HDCore.Functions.CreateCallback("HD-garage:server:GetUserVehicles", function(source, cb, garagename)
  ...
  HDCore.Functions.ExecuteSql(true, "SELECT * FROM `characters_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND garage = '"..garagename.."'", function(result)
      if result ~= nil then
...
HDCore.Functions.CreateCallback("HD-garage:server:GetDepotVehicles", function(source, cb)
  local src = source
  local Player = HDCore.Functions.GetPlayer(src)
  HDCore.Functions.ExecuteSql(true, "SELECT * FROM `characters_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
      if result ~= nil then
          for k, v in pairs(result) do
              cb(result)
          end
...
HDCore.Functions.CreateCallback("HD-garage:server:get:vehicle:mods", function(source, cb, plate)
  local src = source
  local properties = {}
  HDCore.Functions.ExecuteSql(false, "SELECT `mods` FROM `characters_vehicles` WHERE `plate` = '"..plate.."'", function(result)
      if result[1] ~= nil then
...
AddEventHandler('HD-garages:server:set:in:garage', function(Plate, GarageData, Status, MetaData)
 ...
 HDCore.Functions.ExecuteSql(true, "UPDATE `characters_vehicles` SET garage = '" ..GarageData.. "', state = '"..Status.."', metadata = '" ..json.encode(MetaData).. "' WHERE `plate` = '"..PlateEscapeSqli(Plate).."'")
end)
...
AddEventHandler('HD-garages:server:set:in:impound', function(Plate)
 ...
 HDCore.Functions.ExecuteSql(true, "UPDATE `characters_vehicles` SET garage = 'Police', state = 'in', metadata = '" ..json.encode(MetaData).. "' WHERE `plate` = '"..PlateEscapeSqli(Plate).."'")
end)
...
AddEventHandler('HD-garages:server:set:garage:state', function(Plate, Status)
  HDCore.Functions.ExecuteSql(true, "UPDATE `characters_vehicles` SET state = '"..Status.."' WHERE `plate` = '"..PlateEscapeSqli(Plate).."'")
end)
...
AddEventHandler('HD-garages:server:set:depot:price', function(Plate, Price)
  HDCore.Functions.ExecuteSql(true, "UPDATE `characters_vehicles` SET depotprice = '"..Price.."' WHERE `plate` = '"..PlateEscapeSqli(Plate).."'")
end)
