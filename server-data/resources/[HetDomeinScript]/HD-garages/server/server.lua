...
HDCore.Functions.CreateCallback("HD-garage:server:GetPoliceVehicles", function(source, cb, HouseId)
  HDCore.Functions.ExecuteSql(true, "SELECT * FROM `player_vehicles` WHERE `garage` = 'Police'", function(result)
    if result ~= nil then
      cb(result)
    end 
  end)
