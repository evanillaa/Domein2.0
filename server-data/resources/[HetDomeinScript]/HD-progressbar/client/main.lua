...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            HDCore.Functions.Notify("You are already doing something", "error")
        end
    else
        HDCore.Functions.Notify("Can not perform action", "error")
    end
end
                    ...
                    if Action.animation ~= nil then
                        ...
                        elseif Action.animation.animDict ~= nil and Action.animation.anim ~= nil then
                            ...
                            if (DoesEntityExist(player) and not IsEntityDead(player)) then
                                exports['HD-assets']:RequestAnimationDict(Action.animation.animDict)
...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or Action.useWhileDead then
        if not isDoingAction then
            ...
            HDCore.Functions.Notify("You are already working on something...", "error")
        end
    else
        HDCore.Functions.Notify("Something went wrong!", "error")
    end
end
