...
Citizen.CreateThread(function()
    Citizen.Wait(1000)
    for k, v in pairs(HDCore.Shared.Vehicles) do
        if v["shop"] == "custom" then
            for cat,_ in pairs(CustomVehicleCats) do
                if HDCore.Shared.Vehicles[k]["category"] == cat then
                    table.insert(CustomVehicleCats[cat].vehicles, HDCore.Shared.Vehicles[k])
                end
            end
        end
            ...
            if dist < 2 then
                if PlayerJob ~= nil then
                    if PlayerJob.name == "cardealer" then
                        DrawText3Ds(PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.9, '~g~G~w~ - Voertuig veranderen')
                        DrawText3Ds(PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.75, '~b~/verkoop [id]~w~ - Voertuig verkopen ~b~/testrit~w~ - Testrit maken')
                                        ...
                                        if selected then
                                            if IsControlJustPressed(1, 18) then
                                                if CustomModelLoaded then
                                                    TriggerServerEvent('HD-vehicleshop:server:SetCustomShowroomVeh', button.model, ClosestCustomVehicle)
                                                end
                                            end
                                        end
                                    end
                        ...
                        if ClosestCustomVehicle ~= nil then
                            if PepeCustoms.ShowroomPositions[ClosestCustomVehicle] ~= nil then
                                if PepeCustoms.ShowroomPositions[ClosestCustomVehicle].buying then
                                    if HDCore.Shared.Vehicles[PepeCustoms.ShowroomPositions[ClosestCustomVehicle].vehicle] ~= nil then
                                        DrawText3Ds(PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.6, '~g~7~w~ - Kopen / ~r~8~w~ - Annuleren - ~g~(€'..HDCore.Shared.Vehicles[PepeCustoms.ShowroomPositions[ClosestCustomVehicle].vehicle].price..',-)')
                                        
                                        if IsDisabledControlJustPressed(0, Keys["7"]) then
                                            TriggerServerEvent('HD-vehicleshop:server:ConfirmVehicle', PepeCustoms.ShowroomPositions[ClosestCustomVehicle])
                                            PepeCustoms.ShowroomPositions[ClosestCustomVehicle].buying = false
                                        end

                                        if IsDisabledControlJustPressed(0, Keys["8"]) then
                                            HDCore.Functions.Notify('Je hebt de auto NIET gekocht!')
                                            PepeCustoms.ShowroomPositions[ClosestCustomVehicle].buying = false
                                        end
                                    end
                                end
...
RegisterNetEvent('HD-vehicleshop:client:SetCustomShowroomVeh')
AddEventHandler('HD-vehicleshop:client:SetCustomShowroomVeh', function(showroomVehicle, k)
    CancelEvent()
    if PepeCustoms.ShowroomPositions[k].vehicle ~= showroomVehicle then
        HDCore.Functions.DeleteVehicle(GetClosestVehicle(PepeCustoms.ShowroomPositions[k].coords.x, PepeCustoms.ShowroomPositions[k].coords.y, PepeCustoms.ShowroomPositions[k].coords.z, 3.0, 0, 70))
...
RegisterNetEvent('HD-vehicleshop:client:ConfirmVehicle')
AddEventHandler('HD-vehicleshop:client:ConfirmVehicle', function(Showroom, plate)
    HDCore.Functions.SpawnVehicle(Showroom.vehicle, function(veh)
...
RegisterNetEvent('HD-vehicleshop:client:DoTestrit')
AddEventHandler('HD-vehicleshop:client:DoTestrit', function(plate)
    if ClosestCustomVehicle ~= 0 then
        HDCore.Functions.SpawnVehicle(PepeCustoms.ShowroomPositions[ClosestCustomVehicle].vehicle, function(veh)
            TaskWarpPedIntoVehicle(GetPlayerPed(-1), veh, -1)
            exports['HD-fuel']:SetFuelLevel(veh, GetVehicleNumberPlateText(veh), 100, false)
            SetVehicleNumberPlateText(veh, plate)
            SetEntityAsMissionEntity(veh, true, true)
...
RegisterNetEvent('HD-vehicleshop:client:SellCustomVehicle')
AddEventHandler('HD-vehicleshop:client:SellCustomVehicle', function(TargetId)
    local ped = GetPlayerPed(-1)
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if VehicleDist < 2.5 then
            TriggerServerEvent('HD-vehicleshop:server:SellCustomVehicle', TargetId, ClosestCustomVehicle)
        else
            HDCore.Functions.Notify("Je bent niet bij het voertuig in de buurt!", "error")
        end
    else
...
RegisterNetEvent('HD-vehicleshop:client:SetVehicleBuying')
AddEventHandler('HD-vehicleshop:client:SetVehicleBuying', function(slot)
