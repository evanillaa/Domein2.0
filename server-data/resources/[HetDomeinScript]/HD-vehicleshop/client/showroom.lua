...
function CloseCreator(name, veh, price, financed)
	Citizen.CreateThread(function()
        ...
        TriggerServerEvent('HD-vehicleshop:server:setShowroomCarInUse', ClosestVehicle, false)
	end)
end
...
function SelectVehicle(vehicleData)
    TriggerServerEvent('HD-vehicleshop:server:setShowroomVehicle', vehicleData, ClosestVehicle)
    close()
end
...
Citizen.CreateThread(function()
    ...
    for i = 1, #Pepe.ShowroomVehicles, 1 do
        ...
        if oldVehicle ~= 0 then
            HDCore.Functions.DeleteVehicle(oldVehicle)
        end
    ...
    while true do
        ...
        if SellDistance < 20 then
            if IsPedInAnyVehicle(ped) then
                local VehicleData = HDCore.Shared.Vehicles[GetVehiclePedIsIn(ped)]
                if VehicleData["shop"] == "pdm" then
                    ...
                    if SellDistance < 1 then
                        if not SellStarted then
                            DrawText3Ds(Pepe.QuickSell.x, Pepe.QuickSell.y, Pepe.QuickSell.z, '~b~E~w~ - Verkoop voertuig voor ~g~€'..math.ceil(VehicleData["price"] / 100 * 60))
                            if IsControlJustPressed(0, Keys["E"]) then
                                SellStarted = true
                            end
                        else
                            DrawText3Ds(Pepe.QuickSell.x, Pepe.QuickSell.y, Pepe.QuickSell.z, '~b~7~w~ - Bevestig / ~b~8~w~ - Annuleer')
                            if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                                SellStarted = false
                                HDCore.Functions.TriggerCallback('HD-vehicleshop:server:SellVehicle', function(SoldVehicle)
                                    if SoldVehicle then
                                        TriggerEvent("HD-phone-new:client:BankNotify", "Er is €"..math.ceil(VehicleData["price"] / 100 * 60).." bijgeschreven")
                                        local veh = GetVehiclePedIsIn(ped)
                                        HDCore.Functions.DeleteVehicle(veh)
                                    else
                                        HDCore.Functions.Notify('Dit is niet jouw voertuig..', 'error')
                                    end
        ...
        if ClosestVehicle ~= nil then
            if dist < 1.5 then
                if not Pepe.ShowroomVehicles[ClosestVehicle].inUse then
                    ...
                    local displayName = HDCore.Shared.Vehicles[Pepe.ShowroomVehicles[ClosestVehicle].chosenVehicle]["name"]
                    local vehPrice = HDCore.Shared.Vehicles[Pepe.ShowroomVehicles[ClosestVehicle].chosenVehicle]["price"]

                    if not Pepe.ShowroomVehicles[ClosestVehicle].inUse then
                        if not vehshop.opened then
                            if not buySure then
                                DrawText3Ds(Pepe.ShowroomVehicles[ClosestVehicle].coords.x, Pepe.ShowroomVehicles[ClosestVehicle].coords.y, Pepe.ShowroomVehicles[ClosestVehicle].coords.z + 1.8, '~g~G~w~ - Verander voertuig (~g~'..displayName..'~w~)')
                            end
                            if not buySure then
                                DrawText3Ds(Pepe.ShowroomVehicles[ClosestVehicle].coords.x, Pepe.ShowroomVehicles[ClosestVehicle].coords.y, Pepe.ShowroomVehicles[ClosestVehicle].coords.z + 1.70, '~g~E~w~ - Koop voertuig (~g~€'..vehPrice..'~w~)')
                            elseif buySure then
                                DrawText3Ds(Pepe.ShowroomVehicles[ClosestVehicle].coords.x, Pepe.ShowroomVehicles[ClosestVehicle].coords.y, Pepe.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'Weet je het zeker? | ~g~[7]~w~ Ja -/- ~r~[8]~w~ Nee')
                            end
                        elseif vehshop.opened then
                            if modelLoaded then
                                DrawText3Ds(Pepe.ShowroomVehicles[ClosestVehicle].coords.x, Pepe.ShowroomVehicles[ClosestVehicle].coords.y, Pepe.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'Voertuig aan het kiezen')
                            else
                                DrawText3Ds(Pepe.ShowroomVehicles[ClosestVehicle].coords.x, Pepe.ShowroomVehicles[ClosestVehicle].coords.y, Pepe.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'Voertuig wordt opgehaald..')
                            end
                        end
                    else
                        DrawText3Ds(Pepe.ShowroomVehicles[ClosestVehicle].coords.x, Pepe.ShowroomVehicles[ClosestVehicle].coords.y, Pepe.ShowroomVehicles[ClosestVehicle].coords.z + 1.65, 'Voertuig is in gebruik door een klant...')
                    end
                                ...
                                if isValidMenu(vehshop.currentmenu) then
                                    if selected then
                                        if IsControlJustPressed(1, 18) then
                                            if modelLoaded then
                                                TriggerServerEvent('HD-vehicleshop:server:setShowroomVehicle', button.model, ClosestVehicle)
                                            end
                                        end
                                    end
                                end
                    ...
                    if IsDisabledControlJustPressed(0, Keys["7"]) then
                        if buySure then
                            local class = HDCore.Shared.Vehicles[Pepe.ShowroomVehicles[ClosestVehicle].chosenVehicle]["category"]
                            TriggerServerEvent('HD-vehicleshop:server:buyShowroomVehicle', Pepe.ShowroomVehicles[ClosestVehicle].chosenVehicle, class)
                            buySure = false
                        end
                    end
                    if IsDisabledControlJustPressed(0, Keys["8"]) then
                        HDCore.Functions.Notify('Je hebt het voertuig niet gekocht', 'error', 3500)
                        buySure = false
                    end
...
RegisterNetEvent('HD-vehicleshop:client:setShowroomCarInUse')
AddEventHandler('HD-vehicleshop:client:setShowroomCarInUse', function(showroomVehicle, inUse)
    Pepe.ShowroomVehicles[showroomVehicle].inUse = inUse
end)

RegisterNetEvent('HD-vehicleshop:client:setShowroomVehicle')
AddEventHandler('HD-vehicleshop:client:setShowroomVehicle', function(showroomVehicle, k)
    if Pepe.ShowroomVehicles[k].chosenVehicle ~= showroomVehicle then
        HDCore.Functions.DeleteVehicle(GetClosestVehicle(Pepe.ShowroomVehicles[k].coords.x, Pepe.ShowroomVehicles[k].coords.y, Pepe.ShowroomVehicles[k].coords.z, 3.0, 0, 70))
        modelLoaded = false
        Wait(250)
...
RegisterNetEvent('HD-vehicleshop:client:buyShowroomVehicle')
AddEventHandler('HD-vehicleshop:client:buyShowroomVehicle', function(vehicle, plate)
    HDCore.Functions.SpawnVehicle(vehicle, function(veh)
