            ...
            if IsInAppartment then
              local PlayerCoords = GetEntityCoords(GetPlayerPed(-1))
                ...
                if (GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, Config.Locations[CurrentAppartment]['Enter']['X'] - OffSets.stash.x, Config.Locations[CurrentAppartment]['Enter']['Y'] - OffSets.stash.y, Config.Locations[CurrentAppartment]['Enter']['Z'] - OffSets.stash.z, true) < 1.0) then
                    ...
                    if IsControlJustReleased(0, 38) then
                        Other = {maxweight = 1500000, slots = 20}
                        TriggerEvent("HD-inventory:client:SetCurrentStash", HDCore.Functions.GetPlayerData().metadata['appartment-data'].Id)
                        TriggerServerEvent("HD-inventory:server:OpenInventory", "stash", HDCore.Functions.GetPlayerData().metadata['appartment-data'].Id, Other)
                ...
                if (GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, Config.Locations[CurrentAppartment]['Enter']['X'] - OffSets.closet.x, Config.Locations[CurrentAppartment]['Enter']['Y'] - OffSets.closet.y, Config.Locations[CurrentAppartment]['Enter']['Z'] - OffSets.closet.z, true) < 1.2) then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        -- TriggerServerEvent("HD-outfits:server:openUI", true) -- Raid Clothing
                    end
                end
                -- // Log Uit \\ --
...
RegisterNetEvent('HD-appartments:client:enter:appartment')
AddEventHandler('HD-appartments:client:enter:appartment', function(IsNew, AppartmentName)
    if IsNew then
        Citizen.SetTimeout(450, function()
            local Appartment = {}
            local PlayerCoords = GetEntityCoords(GetPlayerPed(-1))
            ...
            if HDCore.Functions.GetPlayerData().metadata['appartment-tier'] == 1 then
                Appartment = exports['HD-interiors']:CreateMidAppartement(CoordsTable)
            else
                Appartment = exports['HD-interiors']:CreateMidHotel(CoordsTable)
            end
            NetworkSetEntityInvisibleToNetwork(PlayerPedId(), true)
            SetEntityHealth(PlayerPedId(), 200.0)
            TriggerEvent('HD-weathersync:client:DisableSync')
            TriggerEvent('HD-clothing:client:CreateFirstCharacter')
            IsInAppartment = true
...
RegisterNetEvent('HD-appartments:client:open:appartment:stash')
AddEventHandler('HD-appartments:client:open:appartment:stash', function(AppartmentId)
    TriggerServerEvent("HD-inventory:server:OpenInventory", "stash", AppartmentId)
    TriggerEvent("HD-inventory:client:SetCurrentStash", AppartmentId)
end)
...
function IsNearHouse()
  for k, v in pairs(Config.Locations) do
      ...
      if Distance < 3.0 then
          if k == HDCore.Functions.GetPlayerData().metadata['appartment-data'].Name then
              CurrentAppartment = k
...
function LogoutPlayer()
  ...
  exports['HD-interiors']:DespawnInterior(HouseData, function()
      ...
      TriggerEvent("HD-sound:client:play", "house-door-close", 0.1)
      Citizen.Wait(450)
      TriggerServerEvent('HD-appartments:server:logout')
  end)
end
...
function AddBlipForAppartment()
  HDCore.Functions.GetPlayerData(function(PlayerData)
    local Appartment = HDCore.Functions.GetPlayerData().metadata['appartment-data'].Name
