...
RegisterNetEvent("HD-recycling:removeWarehouseProps")
AddEventHandler("HD-recycling:removeWarehouseProps", function()
    CleanUpArea()
end)
...
Citizen.CreateThread(function ()
    ...
    HDCore.Functions.DrawText3D(Config['delivery'].OutsideLocation.x,Config['delivery'].OutsideLocation.y, Config['delivery'].OutsideLocation.z, _U("closed") .."" .. Config.OpenHour ..":00")
...
if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].InsideLocation.x, Config['delivery'].InsideLocation.y, Config['delivery'].InsideLocation.z, true) < 15 and not IsPedInAnyVehicle(GetPlayerPed(-1), false) and not onDuty then
    ...
    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].InsideLocation.x, Config['delivery'].InsideLocation.y, Config['delivery'].InsideLocation.z, true) < 1.3 then
        HDCore.Functions.DrawText3D(Config['delivery'].InsideLocation.x, Config['delivery'].InsideLocation.y, Config['delivery'].InsideLocation.z + 1, _U("exit"))
        if IsControlJustReleased(0, Keys["E"]) then
            TriggerEvent('HD-recycling:removeWarehouseProps')
            DoScreenFadeOut(500)
            while not IsScreenFadedOut() do
                Citizen.Wait(10)
            end
        ...
        -- On Duty Shiz
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, 995.38,-3096.27,-39.0, true) < 15 and not IsPedInAnyVehicle(GetPlayerPed(-1), false) and CarryPackage == nil then
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, 995.38,-3096.27,-39.0, true) < 1.3 then
                if onDuty then
                    HDCore.Functions.DrawText3D(995.38,-3096.27,-39.0 +0.5, _U("offduty"))
                    else
                    HDCore.Functions.DrawText3D(995.38,-3096.27,-39.0 +0.5, _U("induty"))
                end
                if IsControlJustReleased(0, Keys["E"]) then
                    onDuty = not onDuty
                    if onDuty then
...
Citizen.CreateThread(function()
    while true do
	    ...
	    local dropOff = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),recycleOut["x"],recycleOut["y"],recycleOut["z"],true)
	    local dropOff2 = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),recycleIn["x"],recycleIn["y"],recycleIn["z"],true)
	    local dropOff3 = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),officeOut["x"],officeOut["y"],officeOut["z"],true)
	    local dropOff5 = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),officeIn["x"],officeIn["y"],officeIn["z"],true)
        --if PlayerJob.name == "recycling" then
            ...
            if dropOff3 < 0.7 then
                ...
                if IsControlJustReleased(0,38) then
                    onDuty =true
					SetEntityCoords(GetPlayerPed(-1),officeIn["x"],officeIn["y"],officeIn["z"])
					SetEntityHeading(GetPlayerPed(-1), officeIn["h"])
					Citizen.Wait(1000)
				end
			end
    ...
    while true do
        ...
        --if PlayerJob.name == "recycling" then
            ...
            if dropOff5 < 0.7 then
                    HDCore.Functions.DrawText3D(officeIn["x"],officeIn["y"],officeIn["z"], _U("enterrecycle")) 
                    if IsControlJustReleased(0,38) then
                        onDuty =false
                        SetEntityCoords(GetPlayerPed(-1),officeOut["x"],officeOut["y"],officeOut["z"])
                        SetEntityHeading(GetPlayerPed(-1), officeOut["h"])
                        Citizen.Wait(1000)
				end
            end
        --end
            ...
            if packagePos ~= nil then
                ...
                if carryPackage == nil then
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, packagePos.x,packagePos.y,packagePos.z, true) < 2.3 then
                        HDCore.Functions.DrawText3D(packagePos.x,packagePos.y,packagePos.z+ 1, _U("grab"))
                        if IsControlJustReleased(0, Keys["E"]) then
                            TaskStartScenarioInPlace(GetPlayerPed(-1), "PROP_HUMAN_BUM_BIN", 0, true)
                            HDCore.Functions.Progressbar("pickup_reycle_package", _U("grabbing"), 5000, false, true, {}, {}, {}, {}, function() -- Done
                                ClearPedTasks(GetPlayerPed(-1))
                                PickupPackage()
                            end)
                        end
                    else
                        HDCore.Functions.DrawText3D(packagePos.x, packagePos.y, packagePos.z + 1, "Package")
                    end
                else
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].DropLocation.x, Config['delivery'].DropLocation.y, Config['delivery'].DropLocation.z, true) < 2.0 then
                        HDCore.Functions.DrawText3D(Config['delivery'].DropLocation.x, Config['delivery'].DropLocation.y, Config['delivery'].DropLocation.z, _U("deliver"))
                        if IsControlJustReleased(0, Keys["E"]) then
                            DropPackage()
                            ScrapAnim()
                            
                                TriggerEvent('HD-inventory:client:set:busy', true)
                            HDCore.Functions.Progressbar("deliver_reycle_package", _U("placing"), 5000, false, true, {
                                ...
                                TriggerServerEvent('HD-recycle:getrecyclablematerial')
                                GetRandomPackage()
                                TriggerEvent('HD-inventory:client:set:busy', false)
                            end)
                        end
                    else
                        HDCore.Functions.DrawText3D(Config['delivery'].DropLocation.x, Config['delivery'].DropLocation.y, Config['delivery'].DropLocation.z, _U("handin"))
                    end
                end
            else
                GetRandomPackage()
            ...
            --if PlayerJob.name == "recycling" then
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].PersonalStash.x, Config['delivery'].PersonalStash.y, Config['delivery'].PersonalStash.z, true) < 18.0) then
                    ...
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].PersonalStash.x, Config['delivery'].PersonalStash.y, Config['delivery'].PersonalStash.z, true) < 1.5) then
                            HDCore.Functions.DrawText3D(Config['delivery'].PersonalStash.x, Config['delivery'].PersonalStash.y, Config['delivery'].PersonalStash.z, "~g~E~w~ - Bulk Trade. ")
                        if IsControlJustReleased(0, Keys["E"]) then
                            ogTradeItems()
            ...
            --if PlayerJob.name == "recycling" then
                if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].TradeItems.x, Config['delivery'].TradeItems.y, Config['delivery'].TradeItems.z, true) < 18.0) then
                    ...
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].TradeItems.x, Config['delivery'].TradeItems.y, Config['delivery'].TradeItems.z, true) < 1.5) then
                            HDCore.Functions.DrawText3D(Config['delivery'].TradeItems.x, Config['delivery'].TradeItems.y, Config['delivery'].TradeItems.z, "~g~E~w~ - Use Stash ")
                        if IsControlJustReleased(0, Keys["E"]) then
                            ogRecycleActions2()
                                Menu.hidden = not Menu.hidden
                            end
...
function OpenCompanyStash()
    ClearMenu()
    TriggerServerEvent("HD-inventory:server:OpenInventory", "stash", "recyclejobstash", {
        maxweight = 4000000,
        slots = 500,
    })
    TriggerEvent("HD-inventory:client:SetCurrentStash", "recyclejobstash")
end


function TradeItems()
    ClearMenu()
TriggerServerEvent('HD-recycle:server:TradeItems')
end

function TradeItemsbulk()
    ClearMenu()
TriggerServerEvent('HD-recycle:server:TradeItemsBulk')
end
