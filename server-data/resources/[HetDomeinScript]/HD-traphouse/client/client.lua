...
AddEventHandler("HDCore:Client:OnPlayerLoaded", function()
    Citizen.SetTimeout(1000, function()
        ...
        Citizen.Wait(250)
        HDCore.Functions.TriggerCallback("HD-traphouse:server:get:config", function(config)
            Config = config
        end)
        LoggedIn = true
    end)
                          ...
                          if NpcDistance < 2 then
                              ...
                              if not Robbing then
                                  if not IsEntityDead(TargetPed) then
                                       ...
                                       exports['HD-assets']:RequestAnimationDict("random@mugging3")
                                       ...
                                       TriggerServerEvent('HD-traphouse:server:rob:npc')
...
RegisterNetEvent('HD-traphouse:client:set:selling:state')
AddEventHandler('HD-traphouse:client:set:selling:state', function(bool)
    Config.IsSelling = bool
end)

RegisterNetEvent('HD-traphouse:client:enter')
AddEventHandler('HD-traphouse:client:enter', function()
    Citizen.SetTimeout(450, function()
        HDCore.Functions.TriggerCallback("HD-traphouse:server:is:current:owner", function(IsOwner)
            if IsOwner then
             EnterTrapHouse()
            else
...
RegisterNUICallback('EnterPincode', function(data)
  HDCore.Functions.TriggerCallback('HD-traphouse:server:pin:code', function(Code)
      if tonumber(data.pin) ~= nil then
          if tonumber(data.pin) == Code then
            EnterTrapHouse()
            TriggerServerEvent('HD-traphouse:set:owner')
          else
...
function EnterTrapHouse()
  ...
  OpenDoorAnim()
  Citizen.Wait(350)
  TrapHouse = exports['HD-interiors']:TrapHouse(CoordsTable)
...
function TryToSell()
    HDCore.Functions.TriggerCallback("HD-traphouse:server:has:sell:item", function(HasSellItem)
        if HasSellItem then
            TriggerServerEvent('HD-traphouse:server:set:selling:state', true)
            Citizen.SetTimeout(math.random(0, 10), function()
                TriggerServerEvent('HD-traphouse:server:sell:item')
                TriggerServerEvent('HD-traphouse:server:set:selling:state', false)
            end)
        else
            HDCore.Functions.Notify('You do not have anything of interest for him..', 'error')
        end
    end)
end
