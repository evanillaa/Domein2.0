$(document).on('click', '.accept-button', function(e) {
    ...
    $.post('http://HD-burgershot/Click', JSON.stringify({}))
    if (PriceAmount != '' && PriceAmount != undefined && NoteAmount != '' && NoteAmount != undefined) {
        $.post('http://HD-burgershot/AddPrice', JSON.stringify({Price: PriceAmount, Note: NoteAmount}))
        $.post('http://HD-burgershot/CloseNui', JSON.stringify({}))
        ...
        $.post('http://HD-burgershot/ErrorClick', JSON.stringify({}))
    }
});
...
$(document).on('click', '.payment', function(e) {
    ...
    if (NumberId != null && Note!= null && Price != null) {
        $.post('http://HD-burgershot/PayReceipt', JSON.stringify({Price: Price, Note: Note, Id: NumberId}))
        $.post('http://HD-burgershot/CloseNui', JSON.stringify({}))
        $(".main-container").animate({"top": "-30vh"}, 250, function() {
            $('.items').html('');
            $('.main-container').css("display", "none");
