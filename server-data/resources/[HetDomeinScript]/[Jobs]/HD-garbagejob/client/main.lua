            ...
            if distance < 20 then
                ...
                if distance < 1.5 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        TriggerServerEvent('HD-garbagejob:server:PayShitBakker', Earnings, location)
                        Earnings = 0
                    end
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                HDCore.Functions.TriggerCallback('HD-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        HDCore.Functions.Notify("Je hebt €1000,- borg terug ontvangen!")
                                    else
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                HDCore.Functions.TriggerCallback('HD-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        local coords = Config.Locations["vehicle"].coords
                                        HDCore.Functions.SpawnVehicle("trash", function(veh)
                                            GarbageVehicle = veh
                                        ...
                                        HDCore.Functions.Notify("Je hebt niet genoeg geld voor de borg.. Borg kosten zijn €1000,-")
                                    end
                                end)
                            end
                        end
                                            ...
                                            if IsControlJustPressed(0, 47) then
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        SetGarbageRoute()
                                                        HDCore.Functions.Notify("Alle vuilniszakken zijn gedaan, ga door naar de volgende locatie!")
                                                    else
                                                        -- Hier ben je klaar met werken.
                                                        HDCore.Functions.Notify("Je bent klaar met werken! Ga terug naar de vuilnisbelt.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        HDCore.Functions.Notify("Er zijn nog "..AmountOfBags.." zakken over!")
                                                    else
                                                        HDCore.Functions.Notify("Er is nog "..AmountOfBags.." zak over!")
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()
