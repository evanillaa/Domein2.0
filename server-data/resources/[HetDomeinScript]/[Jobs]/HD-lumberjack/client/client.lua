...
Citizen.CreateThread(function()
	...
	DrawText3D2(procesX, procesY, procesZ+0.1, "[E] Slice wood")
		...
		HDCore.Functions.Notify('You dont have any planks.', 'error')
		end
		end	
	end
...
function SellKurczaki22()
    ...
    local inventory = HDCore.Functions.GetPlayerData().inventory
    local count = 0
    ----
    
    if(count == 0) then
    HDCore.Functions.Progressbar("search_register", "Slicing wood..", 5000, false, true, {disableMovement = true,
...
function SellKurczaki()
    ...
    ----
    if(count == 0) then
    HDCore.Functions.Progressbar("search_register", "Selling..", 5000, false, true, {disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        disableInventory = true,
...
-------genera auto
RegisterNetEvent('HD-jobwood:Auto')
AddEventHandler('HD-jobwood:Auto', function(boatModel, plate)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    HDCore.Functions.SpawnVehicle(boatModel, function(veh)
    ...
    while true do
        ...
        if dist <= 1.0 then
		    ...
		    if GetPedInVehicleSeat(vehicled, -1) == PlayerPedId() then
				...
				local s1 = false
                    HDCore.Functions.Notify('I\'m preparing your car, 100% fuel.', 'success',7000)
                    HDCore.Functions.Progressbar("search_register", "Inserting code card..", 7000, false, true, {
                        ...
                        while timeLeft > 0 do
                          ...
                          TriggerServerEvent('HD-jobwood:server:truck', model1, ClosestBerth)
                        end
                    end, function()
		...
		if dist <= 2.5 then
            ...
            if GetPedInVehicleSeat(vehicled, -1) == PlayerPedId() then
                ...
                if IsControlJustPressed(0, Keys['E']) then if HasVehicle then
                    ...
                    HDCore.Functions.DeleteVehicle(veh2)
                    HasVehicle = false
