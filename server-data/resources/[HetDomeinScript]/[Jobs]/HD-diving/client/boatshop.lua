...
function SpawnBerthBoats()
    for loc,_ in pairs(QBBoatshop.Locations["berths"]) do
        if SpawnedBoats[loc] ~= nil then
            HDCore.Functions.DeleteVehicle(SpawnedBoats[loc])
        end
            ...
            if BuyDistance < 2 then                
                ...
                if not Buying then
                    ...
                    if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustReleased(0, Keys["7"]) then
                        TriggerServerEvent('HD-diving:server:BuyBoat', QBBoatshop.Locations["berths"][ClosestBerth]["boatModel"], ClosestBerth)
                        Buying = false
...
RegisterNetEvent('HD-diving:client:BuyBoat')
AddEventHandler('HD-diving:client:BuyBoat', function(boatModel, plate)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Water Haven")
