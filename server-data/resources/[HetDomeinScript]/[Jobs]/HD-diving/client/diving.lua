...
RegisterNetEvent('HD-diving:client:NewLocations')
AddEventHandler('HD-diving:client:NewLocations', function()
    HDCore.Functions.TriggerCallback('HD-diving:server:GetDivingConfig', function(Config, Area)
        QBDiving.Locations = Config
        TriggerEvent('HD-diving:client:SetDivingLocation', Area)
    end)
end)

RegisterNetEvent('HD-diving:client:SetDivingLocation')
AddEventHandler('HD-diving:client:SetDivingLocation', function(DivingLocation)
                            ...
                            if not CoralLocation.PickedUp then
                                ...
                                if CoralDistance <= 1.5 then
                                    ...
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        ...
                                        HDCore.Functions.Progressbar("take_coral", "Koraal aan het verzamelen..", times * 1000, false, true, {
...
function TakeCoral(coral)
    ...
    TriggerServerEvent('HD-diving:server:TakeCoral', CurrentDivingLocation.Area, coral, true)
end

RegisterNetEvent('HD-diving:client:UpdateCoral')
AddEventHandler('HD-diving:client:UpdateCoral', function(Area, Coral, Bool)
...
function CallCops()
    ...
    if Call == Chance then
        TriggerServerEvent('HD-diving:server:CallCops', Coords)
    end
end

RegisterNetEvent('HD-diving:server:CallCops')
AddEventHandler('HD-diving:server:CallCops', function(Coords, msg)
...
AddEventHandler('HD-diving:client:UseGear', function(bool)
    if bool then
        ...
        HDCore.Functions.Progressbar("equip_gear", "Duikpak aantrekken..", 5000, false, true, {}, {}, {}, {}, function() -- Done
            ...
            SetPedMaxTimeUnderwater(PlayerPedId(), 18000.00)
