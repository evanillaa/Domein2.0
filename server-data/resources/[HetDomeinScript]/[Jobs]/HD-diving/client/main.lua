...
AddEventHandler("HDCore:Client:OnPlayerLoaded", function()
    HDCore.Functions.TriggerCallback('HD-diving:server:GetBusyDocks', function(Docks)
...
RegisterNetEvent('HD-diving:client:UseJerrycan')
AddEventHandler('HD-diving:client:UseJerrycan', function()
    local ped = PlayerPedId()
    local boat = IsPedInAnyBoat(ped)
    if boat then
        local curVeh = GetVehiclePedIsIn(ped, false)
        HDCore.Functions.Progressbar("reful_boat", "Aan het vol tanken..", 20000, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        }, {}, {}, {}, function() -- Done
            exports['HD-fuel']:SetFuelLevel(curVeh, GetVehicleNumberPlateText(curVeh), 100, false)
            HDCore.Functions.Notify('Het voertuig is getankt', 'success')
            TriggerServerEvent('HD-diving:server:RemoveItem', 'jerry_can', 1)
            TriggerEvent('inventory:client:ItemBox', HDCore.Shared.Items['jerry_can'], "remove")
        end, function() -- Cancel
            HDCore.Functions.Notify('Tanken is geannuleerd!', 'error')
        end)
    else
        HDCore.Functions.Notify('Je zit niet in een boot', 'error')
    end
end)
