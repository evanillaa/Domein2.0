...
--     if PlayerJob.name == "butcher" then 
--         while HDCore.Functions.GetPlayerData().job == nil do Wait(0) end
...
function TepnijWyjscie()
	...
	if DoesEntityExist(vehicle) then
		...
		TriggerServerEvent("HD-pig:getNewPig")
		end
	end
end
...
function TepnijWyjscie1()
	...
	if DoesEntityExist(vehicle) then
			...
			TriggerServerEvent("HD-pig:getNewBeef")
		end
	end
end
...
function packingg(stanowisko)
	local inventory =  HDCore.Functions.GetPlayerData()
	...
	HDCore.Functions.TriggerCallback('HDCore:HasItem', function(result)
		if result then
			...
			HDCore.Functions.Progressbar("wash-", "Packing..", 12000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			}, {}, {}, {}, function()
			TriggerServerEvent("HD-pig:getpackedPig", 2)
...
function packingg1(stanowisko)
	...
	HDCore.Functions.TriggerCallback('HDCore:HasItem', function(result)
		if result then
			...
			TriggerServerEvent("HD-pig:getpackedBeef", 2)
			HDCore.TriggerEvent("DoLongHudText", "Keep packing the beef or go to the vehicle and store it.")
			ClearPedTasks(PlayerPedId())
			DeleteEntity(karton)
			DeleteEntity(mieso)
			end, function() -- Cancel
		...
		HDCore.TriggerEvent("DoShortHudText", "You have nothing to pack!", 2)
		end
	end, 'slaughteredbeef')
end
...
function portioning(position)
	...
	HDCore.Functions.TriggerCallback('HDCore:HasItem', function(result)
		...
		if position == 1 then
			...
			HDCore.Functions.Progressbar("Cut-", "Slaughtering..", 12000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			...
			TriggerServerEvent("HD-pig:getcutPig", 1)
			FreezeEntityPosition(PlayerPedId(),false)
			DeleteEntity(nozyk)
			end, function() -- Cancel
				FreezeEntityPosition(PlayerPedId(),false)
	...
	HDCore.Functions.TriggerCallback('HDCore:HasItem', function(result)
		...
		if position == 1 then
			...
			HDCore.Functions.Progressbar("Cut-", "Slaughtering..", 12000, false, true, {
				...
				TriggerServerEvent("HD-pig:getcutBeef", 1)
				FreezeEntityPosition(PlayerPedId(),false)
				DeleteEntity(nozyk)
			end, function() -- Cancel
				FreezeEntityPosition(PlayerPedId(),false)
