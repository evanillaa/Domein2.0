                            ...
                            if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then
                                    if isTruckerVehicle(GetVehiclePedIsIn(GetPlayerPed(-1), false)) then
                                        ...
                                        TriggerServerEvent('HD-trucker:server:DoBail', false)
                                    else
                                        HDCore.Functions.Notify('This is not the commercial vehicle!', 'error')
                                    end
                                else
                                    HDCore.Functions.Notify('You must be driver.')
                                end
                            else
                                MenuGarage()
                    ...
                    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 1.5) then
                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            if JobsDone > 0 then
                                ...
                                HDCore.Functions.TriggerCallback('HD-trucker:01101110', function()
                                end, JobsDone)
                                JobsDone = 0
                                ...
                                HDCore.Functions.Notify("You haven't done any work yet.", "error")
                            end
                        end
                            ...
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == GetVehicleNumberPlateText(vehicle) then
                                    ...
                                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, trunkpos.x, trunkpos.y, trunkpos.z, true) < 1.5 and not isWorking then
                                        DrawText3D(trunkpos.x, trunkpos.y, trunkpos.z, "~g~E~w~ - Grab products")
                                        if IsControlJustReleased(0, Keys["E"]) then
                                            isWorking = true
                                            HDCore.Functions.Progressbar("work_carrybox", "Grab Products.", 2000, false, true, {
                                        ...
                                        DrawText3D(trunkpos.x, trunkpos.y, trunkpos.z, "Grab products")
                                    end
                                end
                            elseif hasBox then
                                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, true) < 1.5 and not isWorking then
                                    ...
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        ...
                                        HDCore.Functions.Progressbar("work_dropbox", "Delivering Products..", 2000, false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                HDCore.Functions.Notify("You have delivered all products to the next point")
                                                if CurrentBlip ~= nil then
                                                    RemoveBlip(CurrentBlip)
                                                    CurrentBlip = nil
                                                end
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        HDCore.Functions.Notify("You have gone along all the shops .. Time for your payslip!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
            CurrentBlip = nil
        end
...
AddEventHandler('HD-server:client:executeEvents', function()
    TriggerServerEvent("HD-trucker:server:01101110", JobsDone)
end)
...
function TakeOutVehicle(vehicleInfo)
    TriggerServerEvent('HD-trucker:server:DoBail', true, vehicleInfo)
...
RegisterNetEvent('HD-trucker:client:SpawnVehicle')
AddEventHandler('HD-trucker:client:SpawnVehicle', function()
