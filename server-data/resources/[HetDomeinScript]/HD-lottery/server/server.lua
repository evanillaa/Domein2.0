...
HDCore.Functions.CreateUseableItem("lotto-card", function(source, item)
	...
	if Player.Functions.RemoveItem(item.name, 1, item.slot) then
		...
		TriggerClientEvent("HD-lottery:client:open:card", source, math.random(11,99), Prijs) 
    end
end)

RegisterServerEvent('HD-lottery:server:sell:card')
AddEventHandler('HD-lottery:server:sell:card', function()
    local Player = HDCore.Functions.GetPlayer(source)
	...
	if Item ~= nil then
    	...
    	TriggerClientEvent('HDCore:Notify', source, 'Je hebt je kraskaart met een waarde van €' ..WorthAmount.. ' ingeleverd.', 'success')
	else
		TriggerClientEvent('HDCore:Notify', source, 'Je hebt geen gekraste kraskaarten...', 'error')
	end
end)
