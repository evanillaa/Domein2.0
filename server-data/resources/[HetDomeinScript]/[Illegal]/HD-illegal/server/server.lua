...
HDCore.Functions.CreateCallback('HD-illegal:server:get:config', function(source, cb)
    cb(Config)
end)
...
HDCore.Functions.CreateCallback('HD-illegal:serverhas:robbery:item', function(source, cb)
...
HDCore.Functions.CreateCallback('HD-illegal:server:has:drugs', function(source, cb)
    local src = source
...
HDCore.Functions.CreateCallback('HD-illegal:server:get:drugs:items', function(source, cb)
    local src = source
...
RegisterServerEvent('HD-illegal:server:unpack:coke')
AddEventHandler('HD-illegal:server:unpack:coke', function()
    local Player = HDCore.Functions.GetPlayer(source)
    if Player.Functions.RemoveItem('packed-coke-brick', 1) then
        Player.Functions.AddItem('pure-coke-brick', math.random(2, 3))
        TriggerClientEvent('HD-inventory:client:ItemBox', source, HDCore.Shared.Items['pure-coke-brick'], "add")
        TriggerClientEvent('HD-inventory:client:ItemBox', source, HDCore.Shared.Items['packed-coke-brick'], "remove")
    end
end)

RegisterServerEvent('HD-illegal:server:finish:corner:selling')
AddEventHandler('HD-illegal:server:finish:corner:selling', function(Price, ItemName, ItemAmount)
    local Player = HDCore.Functions.GetPlayer(source)
    if Player.Functions.RemoveItem(ItemName, ItemAmount) then
        Player.Functions.AddMoney('cash', Price, 'corner-selling')
        TriggerClientEvent('HD-inventory:client:ItemBox', source, HDCore.Shared.Items[ItemName], "remove")
    end
end)
                ...
                SetTimeout(SellData['Wait'], function()
                    if Config.Labs[LabId]['Inventory'][Slot] ~= nil then
                        ...
                        if SellData['Success'] == 'coke-powder' then
                            TriggerClientEvent('HD-illegal:client:play:sound:coke', -1)
                        end
                        TriggerClientEvent('HD-illegal:client:sync:inventory', -1, LabId, Config.Labs[LabId]['Inventory'])
                    end
                end)
                if Amount > 1 then
...
RegisterServerEvent('HD-illegal:server:add:ingredient')
AddEventHandler('HD-illegal:server:add:ingredient', function(LabId, IngredientName, Bool, Amount)
  ...
  if Config.Labs[LabId]['Ingredients']['meth-ingredient-1'] and Config.Labs[LabId]['Ingredients']['meth-ingredient-2'] then
    ...
    TriggerClientEvent('HD-illegal:client:start:cooking', -1, LabId)
  end
  TriggerClientEvent('HD-illegal:client:sync:meth', -1, Config.Labs[LabId], LabId, false)
end)

RegisterServerEvent('HD-illegal:server:get:meth')
AddEventHandler('HD-illegal:server:get:meth', function(RandomAmount, LabId)
 local src = source
 local Player = HDCore.Functions.GetPlayer(src)
 ResetMethLab(LabId)
 Citizen.SetTimeout(150, function()
    Player.Functions.AddItem('meth-powder', RandomAmount)
    TriggerClientEvent('HD-inventory:client:ItemBox', src, HDCore.Shared.Items['meth-powder'], "add")
 end)
end)

RegisterServerEvent('HD-illegal:server:reset:meth')
AddEventHandler('HD-illegal:server:reset:meth', function(LabId)
    ResetMethLab(LabId)
end)
...
function ResetMethLab(LabId)
 ...
 TriggerClientEvent('HD-illegal:client:sync:meth', -1, Config.Labs[LabId], LabId, true)
end
...
RegisterServerEvent('HD-illegal:server:add:printer:item')
AddEventHandler('HD-illegal:server:add:printer:item', function(LabId, ItemType, Amount)
    Config.Labs[LabId][ItemType] = Config.Labs[LabId][ItemType] + Amount
    TriggerClientEvent('HD-illegal:client:sync:items', -1, ItemType, Config.Labs[LabId][ItemType])
end)

RegisterServerEvent('HD-illegal:server:remove:printer:item')
AddEventHandler('HD-illegal:server:remove:printer:item', function(LabId, ItemType, Amount)
    Config.Labs[LabId][ItemType] = Config.Labs[LabId][ItemType] - Amount
    TriggerClientEvent('HD-illegal:client:sync:items', -1, ItemType, Config.Labs[LabId][ItemType])
end)

RegisterServerEvent('HD-illegal:server:set:printer:money')
AddEventHandler('HD-illegal:server:set:printer:money', function(LabId, Amount)
    Config.Labs[LabId]['Total-Money'] = Config.Labs[LabId]['Total-Money'] + Amount
    TriggerClientEvent('HD-illegal:client:sync:money', -1, Config.Labs[LabId]['Total-Money'])
end)

RegisterServerEvent('HD-illegal:server:get:money:printer:money')
AddEventHandler('HD-illegal:server:get:money:printer:money', function()
    local src = source
    local Player = HDCore.Functions.GetPlayer(src)
    Player.Functions.AddMoney('cash', Config.Labs[3]['Total-Money'], 'money-printer')
    Config.Labs[3]['Total-Money'] = 0
    TriggerClientEvent('HD-illegal:client:sync:money', -1, Config.Labs[3]['Total-Money'])
end)

--- Electrnics 

RegisterServerEvent('HD-illegal:server:sell:electrnoics')
AddEventHandler('HD-illegal:server:sell:electrnoics', function()
    local src = source
    local Player = HDCore.Functions.GetPlayer(src)
    for k, v in pairs(Player.PlayerData.items) do
        if v.name == 'stolen-tv' then
            ...
            TriggerClientEvent('HD-inventory:client:ItemBox', Player.PlayerData.source, HDCore.Shared.Items['stolen-tv'], "remove")
        elseif v.name == 'stolen-micro' then
            Player.Functions.RemoveItem('stolen-micro', 1)
            Player.Functions.AddItem('money-roll', math.random(15, 25))
            Player.Functions.AddMoney('cash', math.random(1500, 2100), 'sold-stolen-goods')
            TriggerClientEvent('HD-inventory:client:ItemBox', Player.PlayerData.source, HDCore.Shared.Items['stolen-micro'], "remove")
        elseif v.name == 'stolen-pc' then
            Player.Functions.RemoveItem('stolen-pc', 1)
            Player.Functions.AddItem('money-roll', math.random(15, 25))
            Player.Functions.AddMoney('cash', math.random(1500, 2100), 'sold-stolen-goods')
            TriggerClientEvent('HD-inventory:client:ItemBox', Player.PlayerData.source, HDCore.Shared.Items['stolen-pc'], "remove")
        end
    end
end)