...
RegisterNetEvent('HD-illegal:client:toggle:corner:selling')
AddEventHandler('HD-illegal:client:toggle:corner:selling', function()
    ...
    if Distance < 1250 then
        HDCore.Functions.TriggerCallback('HD-illegal:server:has:drugs', function(HasDrugs)
            if HasDrugs then
                if Config.IsCornerSelling then
                    CurrentNpc = nil
                    HasNpcFound = false
                    Config.IsCornerSelling = false
                    HDCore.Functions.Notify('Corner Selling staat uit..', 'error')
                elseif not Config.IsCornerSelling then
                    Config.IsCornerSelling = true
                    HDCore.Functions.Notify('Corner Selling staat aan..', 'success')
                end
            else
                CurrentNpc = nil
                HasNpcFound = false
                Config.IsCornerSelling = false
                HDCore.Functions.Notify('Je hebt niet eens stuff man nerd..', 'error')
            end
        end)
    else
        CurrentNpc = nil
        HasNpcFound = false
        Config.IsCornerSelling = false
        HDCore.Functions.Notify('Je bent niet eens in de stad..', 'error')
    end
end)
    ...
    while true do
        ...
        if LoggedIn then
            if Config.IsCornerSelling then
                ...
                if Distance < 1250 then
                    InCornerZone = true
                    local ClosestNpcPed, ClosestNpcDistance = HDCore.Functions.GetClosestPed(PlayerCoords, GetActiveServerPlayers())
                ...
                if not InCornerZone then
                    ...
                    if Config.IsCornerSelling then
                        ...
                        CurrentNpc = nil
                        HasNpcFound = false
                        HDCore.Functions.Notify('Corner Selling uitgeschakeld, Je bent tever van de stad..', 'error')
                    end
                end
            else
                Citizen.Wait(1500)
...
RegisterNetEvent('HD-illegal:client:sell:to:ped')
AddEventHandler('HD-illegal:client:sell:to:ped', function()
    if CurrentNpc ~= nil and Config.IsCornerSelling then
        HDCore.Functions.TriggerCallback('HD-illegal:server:get:drugs:items', function(DrugsResult)
            ...
            if SuccesChance <= 30 then
                ...
                while HasOffer do
                    ...
                    if Distance < 2.0 then
                        DrawText3D(NpcPedCoords.x, NpcPedCoords.y, NpcPedCoords.z, '~g~E~w~ - '..BagAmount..'x '..HDCore.Shared.Items[DrugItemName].label..' voor ~g~€'..RandomPrice..'~s~? / ~g~H~w~ - Aanbod afwijzen')
                    else
                        SetEveryoneIgnorePlayer(PlayerId(), false)
                        Citizen.Wait(250)
                ...
                HasOffer = false
                HasNpcFound = false
                TriggerServerEvent('HD-police:server:alert:cornerselling', GetEntityCoords(CurrentNpc), HDCore.Functions.GetStreetLabel())
                SetEveryoneIgnorePlayer(PlayerId(), false)
                Citizen.Wait(150)
                TaskSmartFleePed(CurrentNpc, GetPlayerPed(-1), 100.0, 15000)
                table.insert(LastPed, CurrentNpc) 
        ...
        HDCore.Functions.Notify('Je bent niet aan het cornersellen...', 'error')
    end
end)
...
function SellNowToPed(Price, Item, Amount)
    ...
    HDCore.Functions.Progressbar("open-brick", "Verkopen..", 7500, false, true, {
        ...
        TriggerServerEvent('HD-illegal:server:finish:corner:selling', Price, Item, Amount)
        StopAnimTask(GetPlayerPed(-1), "oddjobs@assassinate@vice@hooker", "argue_a", 1.0)
        HDCore.Functions.TriggerCallback('HD-illegal:server:has:drugs', function(HasDrugs)
            if not HasDrugs then
                Config.IsCornerSelling = false
                HDCore.Functions.Notify('Je hebt geen spullen meer..', 'error')
            end
        end)
        HasNpcFound = false
        CurrentNpc = nil
