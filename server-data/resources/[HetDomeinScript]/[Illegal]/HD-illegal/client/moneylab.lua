...
function FeedPaper()
    HDCore.Functions.TriggerCallback('HDCore:HasItem', function(HasItem)
        if HasItem then
            TriggerServerEvent('HDCore:Server:RemoveItem', 'money-paper', 1)
            TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items['money-paper'], "remove")
            TriggerServerEvent('HD-illegal:server:add:printer:item', CurrentLab, 'Paper-Count', 1)
        else
...
function FeedInkt()
    HDCore.Functions.TriggerCallback('HDCore:HasItem', function(HasItem)
        if HasItem then
            TriggerServerEvent('HDCore:Server:RemoveItem', 'money-inkt', 1)
            TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items['money-inkt'], "remove")
            TriggerServerEvent('HD-illegal:server:add:printer:item', CurrentLab, 'Inkt-Count', 1)
        else
            HDCore.Functions.Notify('Je mist iets..', 'error')
        end
    end, "money-inkt")
...
function GetTotalMoney()
    if Config.Labs[CurrentLab]['Total-Money'] > 0 then
        TriggerServerEvent('HD-illegal:server:get:money:printer:money')
    end
end
    ...
    while true do
        ...
        if LoggedIn then
            if Config.Labs[3]['Paper-Count'] > 0 and Config.Labs[3]['Inkt-Count'] > 0 then
                TriggerServerEvent('HD-illegal:server:remove:printer:item', CurrentLab, 'Inkt-Count', 1)
                TriggerServerEvent('HD-illegal:server:remove:printer:item', CurrentLab, 'Paper-Count', 1)
                Citizen.Wait(60000)
                TriggerServerEvent('HD-illegal:server:set:printer:money', CurrentLab, math.random(2500, 5000))
                Citizen.Wait(150)
            end
        end
    end
...
RegisterNetEvent('HD-illegal:client:sync:items')
AddEventHandler('HD-illegal:client:sync:items', function(ItemType, ConfigData)
    Config.Labs[3][ItemType] = ConfigData
end)

RegisterNetEvent('HD-illegal:client:sync:money')
AddEventHandler('HD-illegal:client:sync:money', function(ConfigData)
