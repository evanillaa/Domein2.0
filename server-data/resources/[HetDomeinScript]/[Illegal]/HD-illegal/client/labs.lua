        ...
        if LoggedIn then
            ...
            for k,v in pairs(Config.Labs) do
                ...
                if Distance < 2.0 then
                    NearLab = true
                    CurrentLab = k
                    local ItemsNeeded = {[1] = {name = HDCore.Shared.Items[v['KeyName']]["name"], image = HDCore.Shared.Items[v['KeyName']]["image"]}}
...
RegisterNetEvent('HD-illegal:client:use:key')
AddEventHandler('HD-illegal:client:use:key', function(KeyName)
    if not InsideLab then
        if CurrentLab ~= nil then
            if KeyName == Config.Labs[CurrentLab]['KeyName'] then
                EnterCurrentLab()
            else
                HDCore.Functions.Notify('Dit is niet de juiste sleutel..', 'error')
            end
        end
    end
end)

RegisterNetEvent('HD-illegal:client:sync:inventory')
AddEventHandler('HD-illegal:client:sync:inventory', function(LabId, data)
...
function EnterCurrentLab()
 ...
 Citizen.Wait(100)
 SetEntityCoords(GetPlayerPed(-1), Config.Labs[CurrentLab]['Coords']['Exit']['X'], Config.Labs[CurrentLab]['Coords']['Exit']['Y'], Config.Labs[CurrentLab]['Coords']['Exit']['Z'])
 Citizen.Wait(150)
 DoScreenFadeIn(500)
end
...
function ExitCurrentLab()
 ...
 Citizen.Wait(100)
 SetEntityCoords(GetPlayerPed(-1), Config.Labs[CurrentLab]['Coords']['Enter']['X'], Config.Labs[CurrentLab]['Coords']['Enter']['Y'], Config.Labs[CurrentLab]['Coords']['Enter']['Z'])
 Citizen.Wait(150)
 DoScreenFadeIn(500)
 InsideLab = false
end
