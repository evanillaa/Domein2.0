            ...
            if InsideLab and Config.Labs[CurrentLab]['Name'] == 'Methlab' then
                ...
                if Distance < 1.2 then
                    ...
                    if IsControlJustReleased(0, 38) then
		    			...
		    			TriggerServerEvent("HD-inventory:server:OpenInventory", "methcrafting", math.random(1, 99), MethCrafting)
                    end
                end
...
RegisterNetEvent('HD-illegal:client:start:cooking')
AddEventHandler('HD-illegal:client:start:cooking', function()
    Cooking = true
    while Cooking do
        Citizen.Wait(0)
        if Config.CookTimer > 0 then
        ...
        if Config.CookTimer == 0 then
            ...
            if Random <= Config.ExplosionChance then
                if CurrentLab ~= nil then
                    TriggerServerEvent('HD-illegal:server:reset:meth', CurrentLab)
                    ExplosionMethFail()
                end
            end
            Cooking = false
...
RegisterNetEvent('HD-illegal:client:sync:meth')
AddEventHandler('HD-illegal:client:sync:meth', function(ConfigData, LabId, Reset)
...
function AddIngredient()
    if not Config.Labs[CurrentLab]['Ingredients']['meth-ingredient-1'] then
        HDCore.Functions.TriggerCallback('HDCore:HasItem', function(HasItem)
            if HasItem then
                TriggerServerEvent('HDCore:Server:RemoveItem', 'meth-ingredient-1', 1)
                TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items['meth-ingredient-1'], "remove")
                TriggerServerEvent('HD-illegal:server:add:ingredient', CurrentLab, 'meth-ingredient-1', true, 1)
            else
    ...
    elseif not Config.Labs[CurrentLab]['Ingredients']['meth-ingredient-2'] then
        HDCore.Functions.TriggerCallback('HDCore:HasItem', function(HasItem)
            if HasItem then
                TriggerServerEvent('HDCore:Server:RemoveItem', 'meth-ingredient-2', 1)
                TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items['meth-ingredient-2'], "remove")
                TriggerServerEvent('HD-illegal:server:add:ingredient', CurrentLab, 'meth-ingredient-2', true, 1)
            else
                HDCore.Functions.Notify('Je mist iets..', 'error')
            end
...
function GetMeth()
    TriggerServerEvent('HD-illegal:server:get:meth', math.random(25, 30), CurrentLab)
end