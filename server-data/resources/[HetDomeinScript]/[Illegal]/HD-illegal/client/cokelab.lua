                ...
                if Distance < 2.0 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        local CokeLab = {}
                         ...
                         CokeLab.slots = 2
                        TriggerServerEvent("HD-inventory:server:OpenInventory", "lab", CurrentLab, CokeLab)
                    end
                end
            ...
            if InsideLab and Config.Labs[CurrentLab]['Name'] == 'Cokelab' then
                ...
                if Distance < 1.2 then
                    ...
                    if IsControlJustReleased(0, 38) then
		    			...
		    			TriggerServerEvent("HD-inventory:server:OpenInventory", "cokecrafting", math.random(1, 99), CokeCrafting)
                    end
                end
...
RegisterNetEvent('HD-illegal:client:start:burner-call')
AddEventHandler('HD-illegal:client:start:burner-call', function()
    if not DoingSomething then
        ...
        Citizen.SetTimeout(1250, function()
            ...
            repeat
              Time = Time -1
              TriggerEvent("HD-sound:client:play", "death", 0.2)
              Citizen.Wait(3500)
            until Time == 0
            if RandomValue == 1 then
                TriggerEvent("HD-sound:client:play", "call-1", 0.5) 
            ...
            Citizen.SetTimeout(14000, function()
                ...
                TriggerServerEvent('HDCore:Server:RemoveItem', 'burner-phone', 1)
                TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items['burner-phone'], "remove")
                TriggerServerEvent('HD-phone:server:sendNewMail', {
                    ...
                    button = {
                        enabled = true,
                        buttonEvent = "HD-illegal:client:start:pickup:coke",
...
RegisterNetEvent('HD-illegal:client:start:pickup:coke')
AddEventHandler('HD-illegal:client:start:pickup:coke', function(PickupData)
    local DoingJob = true
    ...
    while DoingJob do
        ...
        if Distance <= 2.0 then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                TriggerServerEvent("HD-inventory:server:OpenInventory", "shop", "Cokebrick", ItemShop)
...
RegisterNetEvent('HD-illegal:client:play:sound:coke')
AddEventHandler('HD-illegal:client:play:sound:coke', function()
    if InsideLab and NearAction then
        TriggerEvent('HD-sound:client:play', 'cutting', 0.4)
    end
end)
