        ...
        if PlayerData.job.name == "police" then
            ...
            if (IsPedInAnyVehicle( ped ) ) then 
                ...
                if (IsPoliceVehicle(vehicle)) then
                    ...
                    HDCore.Functions.Notify( "You have to sit in a police vehicle!", "error")
                end 
            else 
                HDCore.Functions.Notify( "You have to sit in a vehicle!", "error")
            end 
        end
    end)
end)
...
AddEventHandler( 'wk:changeRadarLimit', function( speed ) 
    ...
    HDCore.Functions.Notify("Max speed set to: " .. speed .. "km/u")
end )
                ...
                if ( radarInfo.fwdXmit ) then  
                    ...
                    if ( DoesEntityExist( fwdVeh ) and IsEntityAVehicle( fwdVeh ) ) then 
                        ...
                        if plateChecked ~= fwdPlate then
                            HDCore.Functions.TriggerCallback('police:IsPlateFlagged', function(result)
                                if result then
                                    ...
                                    radarInfo.fwdFastLocked = true 
                                    SendNUIMessage( { lockfwdfast = true } )
                                    HDCore.Functions.Notify("Gemarkeerd voertuig gevonden!", "error")
                                end
        ...
        if HDCore ~= nil then 
            ...
            if ( not IsDisabledControlPressed( 0, Keys["LEFTCTRL"] ) and IsDisabledControlJustPressed( 0, Keys["7"] ) ) then 
                if HDCore.Functions.GetPlayerData().job.name == "police" and HDCore.Functions.GetPlayerData().job.onduty then
                    ...
                    ResetFrontAntenna()
                    ResetRearAntenna()
                    HDCore.Functions.Notify("Police anspr reset")
                end
            end 
            ...
            if ( IsDisabledControlJustPressed( 0, Keys["8"] ) ) then 
                if HDCore.Functions.GetPlayerData().job.name == "police" and HDCore.Functions.GetPlayerData().job.onduty then
                    ...
                    if (radarInfo.plateLocked) then
                        ...
                        HDCore.Functions.Notify("Locked license plate: " .. radarInfo.fwdPlate)
                    else
                        SendNUIMessage( { unlockPlate = true } )
                        HDCore.Functions.Notify("License plate unlocked")
                    end
                end
            end
