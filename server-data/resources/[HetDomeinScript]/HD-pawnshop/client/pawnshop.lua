...
function SellGoldBars()
    HDCore.Functions.TriggerCallback('HDCore:HasItem', function(HasGold)
        if HasGold then
            TriggerEvent('HD-inventory:client:set:busy', true)
         HDCore.Functions.Progressbar("sell-gold", "Sell goods...", math.random(5000, 7000), false, true, {
             disableMovement = true,
             disableCarMovement = true,
             disableMouse = false,
             disableCombat = true,
         }, {}, {}, {}, function() -- Done
             TriggerServerEvent('HD-pawnshop:server:sell:gold:bars')
            ...
            HDCore.Functions.Notify("You have no acceptable goods with you.", "error")
            TriggerEvent('HD-inventory:client:set:busy', false)
        end
    end, 'gold-bar')
end
...
function SellGoldItems(PawnId)
    HDCore.Functions.TriggerCallback('HD-pawnshop:server:has:gold', function(HasGold)
        if HasGold then
            TriggerEvent('HD-inventory:client:set:busy', true)
         HDCore.Functions.Progressbar("sell-gold", "Sell goods...", math.random(5000, 7000), false, true, {
             disableMovement = true,
             ...
             TriggerServerEvent('HD-pawnshop:server:sell:gold:items')
             StopAnimTask(GetPlayerPed(-1), "random@shop_robbery", "robbery_action_b", 1.0)
             HDCore.Functions.Notify("Nobody nearby", "error")
             TriggerEvent('HD-inventory:client:set:busy', false)
         end, function() -- Cancel
             StopAnimTask(GetPlayerPed(-1), "random@shop_robbery", "robbery_action_b", 1.0)
             HDCore.Functions.Notify("Canceled.", "error")
