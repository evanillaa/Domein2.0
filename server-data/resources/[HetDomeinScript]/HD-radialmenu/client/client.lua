...
AddEventHandler('HD-radialmenu:client:flip:vehicle', function()
    ...
    if Vehicle ~= 0 and Distance < 1.7 then
        HDCore.Functions.Progressbar("flip-vehicle", "Flippin the freaking car..", math.random(10000, 15000), false, true, {
            ...
            flags = 49,
             ...
             HDCore.Functions.Notify("Succes", "success")
        end, function()
            HDCore.Functions.Notify("Cancelled..", "error")
        end)
    else
        HDCore.Functions.Notify("No vehicle found..", "error")
    end
end)
        ...
        if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            if DoesExtraExist(veh, extra) then 
                if IsVehicleExtraTurnedOn(veh, extra) then
                    ...
                    HDCore.Functions.Notify('Extra ' .. extra .. ' turned off', 'error', 2500)
                else
                    enginehealth = GetVehicleEngineHealth(veh)
                    bodydamage = GetVehicleBodyHealth(veh)
                    ...
                    SetVehicleEngineHealth(veh, enginehealth)
                    SetVehicleBodyHealth(veh, bodydamage)
                    HDCore.Functions.Notify('Extra ' .. extra .. ' turned on', 'success', 2500)
                end    
            else
                HDCore.Functions.Notify('Extra ' .. extra .. ' does not exist on this car', 'error', 2500)
            end
        else
            HDCore.Functions.Notify('Your not a driver for this vehicle!', 'error', 2500)
        end
    end
end)
...
AddEventHandler("HD-radialmenu:client:send:panic:button",function()
  HDCore.Functions.TriggerCallback('HDCore:HasItem', function(HasItem)
      if HasItem then
          ...
          HDCore.Functions.Notify("You dont have a radio..", "error")
      end
  end, "radio")
end)
