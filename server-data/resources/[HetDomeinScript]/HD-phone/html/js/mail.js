...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('http://HD-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('http://HD-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(QB.Phone.Data.PlayerData.charinfo.firstname+"."+QB.Phone.Data.PlayerData.charinfo.lastname+"@HDCore.be");
...
var MonthFormatting = ["Jan", "Feb", "Maart", "April", "Mei", "Juni", "Juli", "Aug", "Sept", "Okt", "Nov", "Dec"];
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        $.post('http://HD-phone/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
        QB.Phone.Notifications.Add("fas fa-ad", "Advertentie", "Je kan geen lege AD plaatsen!", "#ff8f1a", 2000);
    }
});
