...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Portefeuille: "+CryptoData.Portfolio+"  Bit('s)");
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
      if ((Coins > 0) && (Price > 0)) {
       if ((Coins < 11) && (BuyCoins < Coins)) {
        if (QB.Phone.Data.PlayerData.money.bank >= Price) {
            BuyCoins = parseInt(BuyCoins) + parseInt(Coins)
            $.post('http://HD-phone/BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {   
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "Bank", "Er is &euro; "+Price+",- afgeschreven!", "#badc58", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg geld..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg geld..", "#badc58", 1500);
            ...
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je kan max 10  Bits kopen..", "#badc58", 1500);
        }
    } else {
        QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je kan geen min waardes invullen!", "#badc58", 1500);
    }
} else {
    QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Vul alles in!", "#badc58", 1500);
}
});
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        ...
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            ...
            $.post('http://HD-phone/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    QB.Phone.Data.PlayerData.money.bank = parseInt(QB.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    QB.Phone.Notifications.Add("fas fa-university", "Bank", "Er is &euro; "+Price+",- bijgeschreven!", "#badc58", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg  Bits..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg  Bits..", "#badc58", 1500);
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('http://HD-phone/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
                    if (CryptoData == "notenough") {
                        ...
                        QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Dit Wallet-ID bestaat niet!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Je hebt "+Coins+",- overgemaakt naar "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Je kan niet naar jezelf overmaken..", "#badc58", 2500);
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg  Bits..", "#badc58", 1500);
        }
