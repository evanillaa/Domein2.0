...
QB.Phone.Functions.SetupApplications = function(data) {
    ...
    $.each(data.applications, function(i, app){
        ...
        if ((!app.job || app.job === QB.Phone.Data.PlayerJob.name) || QB.Phone.Data.PlayerJob.name === 'judge' && !blockedapp) {
        ...
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $.post('http://HD-phone/GetMentionedTweets', JSON.stringify({}), function(MentionedTweets){
                        QB.Phone.Notifications.LoadMentionedTweets(MentionedTweets)
                    })
                    $.post('http://HD-phone/GetHashtags', JSON.stringify({}), function(Hashtags){
                        QB.Phone.Notifications.LoadHashtags(Hashtags)
                    })
                    if (QB.Phone.Data.IsOpen) {
                        $.post('http://HD-phone/GetTweets', JSON.stringify({}), function(Tweets){
                            QB.Phone.Notifications.LoadTweets(Tweets);
                        });
                    }
                } else if (PressedApplication == "bank") {
                    QB.Phone.Functions.DoBankOpen();
                    $.post('http://HD-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                        QB.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('http://HD-phone/GetInvoices', JSON.stringify({}), function(invoices){
                        QB.Phone.Functions.LoadBankInvoices(invoices);
                    });
                } else if (PressedApplication == "whatsapp") {
                    $.post('http://HD-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
                        QB.Phone.Functions.LoadWhatsappChats(chats);
                    });
                } else if (PressedApplication == "phone") {
                    $.post('http://HD-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                        QB.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('http://HD-phone/GetSuggestedContacts', JSON.stringify({}), function(suggested){
                        QB.Phone.Functions.SetupSuggestedContacts(suggested);
                    });
                    $.post('http://HD-phone/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('http://HD-phone/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('http://HD-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                        QB.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('http://HD-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "crypto") {
                    $.post('http://HD-phone/GetCryptoData', JSON.stringify({
                        crypto: "qbit",
                    }), function(CryptoData){
                        SetupCryptoData(CryptoData);
                    })

                    $.post('http://HD-phone/GetCryptoTransactions', JSON.stringify({}), function(data){
                ...
                } else if (PressedApplication == "occasion") {
                    $.post('http://HD-phone/LoadAutoscout', JSON.stringify({}), function(Cars){
                        QB.Phone.Functions.RefreshAutoscout(Cars);
                    });
                } else if (PressedApplication == "houses") {
                    $.post('http://HD-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    });
                    $.post('http://HD-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                        $(".house-app-mykeys-container").html("");
                        if (Keys.length > 0) {
                            $.each(Keys, function(i, key){
                                var elem = '<div class="mykeys-key" id="keyid-'+i+'"> <span class="mykeys-key-label">' + key.HouseData['Adres'] + '</span> <span class="mykeys-key-sub">Klik om GPS in te stellen</span> </div>';
                    ...
                    $.post('http://HD-phone/GetCurrentLawyers', JSON.stringify({}), function(data){
                        SetupLawyers(data);
                    });
                } else if (PressedApplication == "store") {
                    $.post('http://HD-phone/SetupStoreApps', JSON.stringify({}), function(data){
                        SetupAppstore(data); 
                    });
                } else if (PressedApplication == "trucker") {
                    $.post('http://HD-phone/GetTruckerData', JSON.stringify({}), function(data){
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('http://HD-phone/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
QB.Phone.Notifications.Add = function(icon, title, text, color, timeout) {
    $.post('http://HD-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
            ...
            if (QB.Phone.Notifications.Timeout == undefined || QB.Phone.Notifications.Timeout == null) {
                ...
                QB.Phone.Animations.TopSlideDown(".phone-notification-container", 1000, 8);
                if (icon !== "politie") {
                    $(".notification-icon").html('<i class="'+icon+'"></i>');
                } else {
                    $(".notification-icon").html('<img src="./img/politie.png" class="police-icon-notify">');
                }
                QB.Phone.Animations.BottomSlideUp('.container', 500, -50)
                ...
                QB.Phone.Notifications.Timeout = setTimeout(function(){
                    QB.Phone.Animations.TopSlideUp(".phone-notification-container", 500, -8);
...
QB.Phone.Functions.LoadPhoneData = function(data) {
    ...
    QB.Phone.Functions.RefreshAutoscout(data.PhoneData.Cars);
...
// QB.Phone.Notifications.Add("Nieuwe Tweet", "razerlolololol", "", 4555000);
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            ...
            case "RefreshAdverts":
                ...
                break;
            case "RefreshAutoscout":
                QB.Phone.Functions.RefreshAutoscout(event.data.Cars);
                break;
            case "AddPoliceAlert":
