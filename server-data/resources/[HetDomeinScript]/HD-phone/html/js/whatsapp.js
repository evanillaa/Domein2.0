...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    $.post('http://HD-phone/ClearAlerts', JSON.stringify({
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('http://HD-phone/SendMessage', JSON.stringify({
...
$(document).on('click', '#send-tikkie', function(e){
    e.preventDefault();
    $.post('http://HD-phone/SendTikkie', JSON.stringify({
        ChatNumber: OpenedChatData.number,
        ChatDate: GetCurrentDateKey(),
        ChatMessage: "Hey ik heb je een tikkie gestuurd kijk in je bank app naar het gevraagde betaal bedrag.",
        ChatTime: FormatMessageTime(),
        ChatType: "message",
    }));
});
...
//         if (OpenedChatPicture == null) {
//             $.post('http://HD-phone/GetProfilePicture', JSON.stringify({
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('http://HD-phone/SharedLocation', JSON.stringify({
