...
$(document).on('click', '.bank-app-account', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Bank", "Bankrekening nr. gekopieerd!", "#badc58", 1750);
});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
        ...
        if (!amount - amountData < 0) {
			if (amount > 0) {
				$.post('http://HD-phone/TransferMoney', JSON.stringify({
					iban: iban,
					amount: amount
				}), function(data){
					if (data.CanTransfer) {
						...
						QB.Phone.Notifications.Add("fas fa-university", "Bank", "Je hebt €"+amount+",- overgemaakt!", "#badc58", 1500);
					} else {
						QB.Phone.Notifications.Add("fas fa-university", "Bank", "Je hebt niet genoeg saldo!", "#badc58", 1500);
					}
				})
				QB.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
			} else {
				QB.Phone.Notifications.Add("fas fa-university", "Bank", "Hoeveelheid mag niet lager zijn dan 0", "#badc58", 1500);
			 }
        } else {
            QB.Phone.Notifications.Add("fas fa-university", "Bank", "Je hebt niet genoeg saldo!", "#badc58", 1500);
        }
    } else {
        QB.Phone.Notifications.Add("fas fa-university", "Bank", "Vul alle velden in!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://HD-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                QB.Phone.Notifications.Add("fas fa-university", "Bank", "Je hebt &euro;"+InvoiceData.amount+" betaald!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('http://HD-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        invoiceId: InvoiceData.invoiceid,
    }));
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Bank", "Je hebt het verzoek afgewezen..", "#badc58", 1500);
 } else {
    QB.Phone.Notifications.Add("fas fa-university", "Bank", "Je kan dit verzoek niet weigeren..", "#badc58", 1500);
 }
});
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", " Bank", "Er is geen IBAN gebonden aan dit contact!", "#badc58", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});