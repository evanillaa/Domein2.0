...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('http://HD-phone/ClearMentions');
        }
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TweetElement = '<div class="twitter-tweet" data-twthandler="@'+TwitterHandle.replace(" ", "_")+'">'+(Anonymous ? '' : '<div class="tweet-reply"><i class="fas fa-reply"></i></div>')+
                '<div class="tweet-tweeter">'+(Anonymous ? 'Anoniem' : Tweet.firstName+' '+Tweet.lastName )+' &nbsp;<span>@'+(Anonymous ? 'Anoniem' : TwitterHandle.replace(" ", "_") )+' &middot; '+TimeAgo+'</span></div>'+
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('http://HD-phone/PostNewTweet', JSON.stringify({
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://HD-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
    ...
    if (hashtags !== null) {
        ...
        $.each(hashtags, function(i, hashtag){
            ...
            if (hashtag.messages.length >= MinimumTrending) {
                Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Trending in Los Santos</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' '+TweetHandle+'</div> </div>';
            } else {
                Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Niet trending in Los Santos</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' '+TweetHandle+'</div> </div>';
            }
...
QB.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var naam = (Tweet.anonymous ? 'Anoniem' : Tweet.firstName+' '+Tweet.lastName)
