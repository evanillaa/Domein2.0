...
$(document).on('click', '.racing-race', function(e){
    ...
    var IsRacer = IsInRace(HDCore.Phone.Data.PlayerData.citizenid, RaceData.RaceData.Racers)
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "De race is al gestart..", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            var Locked = '<i class="fas fa-unlock"></i> Nog niet gestart';
            if (race.RaceData.Started) {
                Locked = '<i class="fas fa-lock"></i> Gestart';
            }
            var LapLabel = "";
            if (race.Laps == 0) {
                LapLabel = "SPRINT"
            ...
            var InRace = IsInRace(HDCore.Phone.Data.PlayerData.citizenid, race.RaceData.Racers);
            var Creator = IsCreator(HDCore.Phone.Data.PlayerData.citizenid, race);
...
$(document).on('click', '#join-race', function(e){
    ...
    $.post('http://HD-phone/IsInRace', JSON.stringify({}), function(IsInRace){
        if (!IsInRace) {
            $.post('http://HD-phone/RaceDistanceCheck', JSON.stringify({
                RaceId: Data.RaceId,
                Joined: true,
            }), function(InDistance){
                if (InDistance) {
                    $.post('http://HD-phone/IsBusyCheck', JSON.stringify({
                        check: "editor"
                    }), function(IsBusy){
                        if (!IsBusy) {
                            $.post('http://HD-phone/JoinRace', JSON.stringify({
                                RaceData: Data,
                            }));
                            $.post('http://HD-phone/GetAvailableRaces', JSON.stringify({}), function(Races){
                                SetupRaces(Races);
                            });
                        } else {
                            HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je zit in een editor..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je zit al in een race..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#quit-race', function(e){
    ...
    $.post('http://HD-phone/LeaveRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '#start-race', function(e){
    ...
    $.post('http://HD-phone/StartRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('http://HD-phone/GetTrackData', JSON.stringify({
        RaceId: $(this).attr('id')
    }), function(TrackData){
...
$(document).on('click', '#setup-race', function(e){
    ...
    $.post('http://HD-phone/GetRaces', JSON.stringify({}), function(Races){
...
$(document).on('click', '#create-race', function(e){
    e.preventDefault();
    $.post('http://HD-phone/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
            ...
            if (!data.IsBusy) {
                $.post('http://HD-phone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je zit in een race..", "#1DA1F2");
                    }
                });
            } else {
                HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je bent al een Track aan het maken..", "#1DA1F2");
            }
        } else {
            HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je hebt geen rechten om Race Track's te maken..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        $.post('http://HD-phone/IsAuthorizedToCreateRaces', JSON.stringify({
            TrackName: TrackName
        }), function(data){
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    $.post('http://HD-phone/StartTrackEditor', JSON.stringify({
                        TrackName: TrackName
                    }));
                    ...
                    HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Deze naam is niet beschikbaar..", "#1DA1F2");
                }
            } else {
                HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je hebt geen rechten om Race Track's te maken..", "#1DA1F2");
            }
        ...
        HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je moet een Tracknaam invullen..", "#1DA1F2");
    }
});
...
$(document).on('click', '#setup-race-accept', function(e){
    ...
    $.post('http://HD-phone/HasCreatedRace', JSON.stringify({}), function(HasCreatedRace){
        if (!HasCreatedRace) {
            $.post('http://HD-phone/RaceDistanceCheck', JSON.stringify({
                RaceId: track,
                Joined: false,
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('http://HD-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    $.post('http://HD-phone/SetupRace', JSON.stringify({
                                        RaceId: track,
                                        AmountOfLaps: laps,
                                    }))
                                    $(".racing-overview").animate({
                                    ...
                                    HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Er kunnen momenteel geen races gemaakt worden..", "#1DA1F2");
                                }
                            });
                        } else {
                            HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Vul een hoeveelheid rondes in..", "#1DA1F2");
                        }
                    } else {
                        HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je hebt geen Track gekozen..", "#1DA1F2");
                    }
                }
            })
        } else {
            HDCore.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je hebt al een race actief..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#leaderboards-race', function(e){
    ...
    $.post('http://HD-phone/GetRacingLeaderboards', JSON.stringify({}), function(Races){
        if (Races !== null) {
