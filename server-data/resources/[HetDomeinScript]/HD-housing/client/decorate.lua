...
RegisterNetEvent("HD-housing:client:decorate")
AddEventHandler("HD-housing:client:decorate", function()
	Citizen.Wait(500)
	if IsInHouse then 
		if HasKey then 
...
RegisterNetEvent("HD-housing:server:sethousedecorations")
AddEventHandler("HD-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house]['Decorations'] == nil or next(Config.Houses[house]['Decorations']) == nil then
		HDCore.Functions.TriggerCallback('HD-housing:server:get:decorations', function(result)
...
function SaveDecorations()
	if Currenthouse ~= nil then
		...
		TriggerServerEvent("HD-housing:server:save:decorations", Currenthouse, ObjectList)
	end
end
