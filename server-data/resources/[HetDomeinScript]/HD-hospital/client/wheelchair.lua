...
-- 	HDCore.PlayerData = HDCore.Functions.GetPlayerData()
-- end)

-- RegisterNetEvent('HDCore:Client:OnPlayerLoaded')
-- AddEventHandler('HDCore:Client:OnPlayerLoaded', function(xPlayer)
-- 	Citizen.Wait(5000)
-- 	HDCore.PlayerData = xPlayer
-- end)
	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 1.0 then
				HDCore.Functions.DrawText3Ds(sitCoords, _U("standup"), 0.4)
			...
			if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 1.0 then
				HDCore.Functions.DrawText3Ds(pickupCoords, _U("push"), 0.4)
