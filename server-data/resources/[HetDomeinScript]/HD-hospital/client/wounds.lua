        ...
        if isLoggedIn then
            if not Config.IsDeath then
               for k, v in pairs(Config.BodyHealth) do
                   ...
                   if v['Pain'] then
                      if TotalPain > 1 then
                        HDCore.Functions.Notify("You are in serious pain on several spots..", 'info')
                      else
                        HDCore.Functions.Notify("You are struggling with pain in your "..v['Name']..'..', 'info')
                      end
                    ...
                    elseif not v['Pain'] and v['IsDead'] then
                        if TotalBroken > 1 then
                            HDCore.Functions.Notify("Multiple broken bones..", 'error')
                        else
                            HDCore.Functions.Notify("Your "..v['Name'].. ' is broken..', 'error')
                        end
                        if k == 'HEAD' then
...
RegisterNetEvent('HD-hospital:client:use:bandage')
AddEventHandler('HD-hospital:client:use:bandage', function()
  Citizen.SetTimeout(1000, function()
     exports['HD-assets']:AddProp('HealthPack')
     HDCore.Functions.Progressbar("use_bandage", "Verband omdoen..", 4000, false, true, {
        ...
        disableCarMovement = false,
         ...
         HealRandomBodyPart()
         TriggerServerEvent('HDCore:Server:RemoveItem', 'bandage', 1)
         TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items['bandage'], "remove")
...
RegisterNetEvent('HD-hospital:client:use:health-pack')
AddEventHandler('HD-hospital:client:use:health-pack', function()
    ...
    if Player ~= -1 and Distance < 1.5 then
      if IsTargetDead(GetPlayerServerId(Player)) then
         HDCore.Functions.Progressbar("hospital_revive", "Helping Citizen..", RandomTime, false, true, {
             ...
             TriggerServerEvent("HDCore:Server:RemoveItem", "health-pack", 1)
             TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items["health-pack"], "remove")
             ...
             HDCore.Functions.Notify("You helped the citizen!")
         end, function() -- Cancel
             StopAnimTask(PlayerPedId(), 'mini@cpr@char_a@cpr_str', "cpr_pumpchest", 1.0)
             HDCore.Functions.Notify("Failed!", "error")
         end)
...
RegisterNetEvent('HD-hospital:client:use:painkillers')
AddEventHandler('HD-hospital:client:use:painkillers', function()
    Citizen.SetTimeout(1000, function()
        if not Config.OnOxy then
        HDCore.Functions.Progressbar("use_bandage", "Taking Oxycodons..", 3000, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent("HDCore:Server:RemoveItem", "painkillers", 1)
            TriggerEvent("HD-inventory:client:ItemBox", HDCore.Shared.Items["painkillers"], "remove")
            Config.OnOxy = true
         ...
         HDCore.Functions.Notify("You still have remains ofoxycodon in your body..", "error")
       end 
    end)
end)
...
function ApplyDamageToBodyPart(BodyPart)
    ...
    TriggerServerEvent('HD-police:server:CreateBloodDrop', GetEntityCoords(PlayerPedId()))
end 
