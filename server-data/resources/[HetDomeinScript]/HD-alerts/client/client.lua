...
RegisterNetEvent('HD-alerts:client:send:alert')
AddEventHandler('HD-alerts:client:send:alert', function(data, forBoth)
    if forBoth then
        if (HDCore.Functions.GetPlayerData().job.name == "police" or HDCore.Functions.GetPlayerData().job.name == "ambulance") and HDCore.Functions.GetPlayerData().job.onduty then
            ...
            if data.priority == 1 then
                TriggerServerEvent("HD-sound:server:play:source", "alert-high-prio", 0.2)
            ...
            elseif data.priority == 3 then
                TriggerServerEvent("HD-sound:server:play:source", "alert-panic-button", 0.5)
            else
...
RegisterNUICallback('SetWaypoint', function(data)
    ...
    HDCore.Functions.Notify('GPS ingesteld!', 'success')
end)