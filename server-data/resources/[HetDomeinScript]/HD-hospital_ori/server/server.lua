	...
	for k, v in pairs(HDCore.Functions.GetPlayers()) do
		...
		if Player ~= nil then 
			if ((Player.PlayerData.job.name == "ambulance") and Player.PlayerData.job.onduty) then
				TriggerClientEvent("HD-hospital:client:SendAlert", v, "Er is een dokter nodig bij de balie van het Ziekenhuis")
			end
		end
	end
end)
...
HDCore.Commands.Add("revive", "Revive a player or yourself", {{name="id", help="Player ID (can be empty)"}}, false, function(source, args)
	if args[1] ~= nil then
...
HDCore.Commands.Add("setems", "Hire someone as ems", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = HDCore.Functions.GetPlayer(source)
    local TargetPlayer = HDCore.Functions.GetPlayer(tonumber(args[1]))
    if Player.PlayerData.metadata['ishighcommand'] then
      if TargetPlayer ~= nil then
          TriggerClientEvent('HDCore:Notify', TargetPlayer.PlayerData.source, 'You have been hired as an EMS congrats!', 'success')
          TriggerClientEvent('HDCore:Notify', Player.PlayerData.source, 'You have hired'..TargetPlayer.PlayerData.charinfo.firstname..' '..TargetPlayer.PlayerData.charinfo.lastname..' as EMS!', 'success')
          TargetPlayer.Functions.SetJob('ambulance', 0)
      end
    end
end)

HDCore.Commands.Add("fireems", "Fire a ems personal", {{name="id", help="Player ID"}}, true, function(source, args)
    local Player = HDCore.Functions.GetPlayer(source)
    local TargetPlayer = HDCore.Functions.GetPlayer(tonumber(args[1]))
    if Player.PlayerData.metadata['ishighcommand'] then
      if TargetPlayer ~= nil then
          TriggerClientEvent('HDCore:Notify', TargetPlayer.PlayerData.source, 'You are fired from your last job!', 'error')
          TriggerClientEvent('HDCore:Notify', Player.PlayerData.source, 'You have fired '..TargetPlayer.PlayerData.charinfo.firstname..' '..TargetPlayer.PlayerData.charinfo.lastname..'!', 'success')
