...
StoreRob.Functions.ClosePinpad = function(HasPutin) {
    ...
    $.post('http://HD-wapenkluis/PinpadClose', JSON.stringify({
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        ...
        case 13:
            var v = $("#PINbox").val();
            if (v == "") {
                $.post('http://HD-wapenkluis/ClickFail', JSON.stringify({}))
                $.post('http://HD-wapenkluis/ErrorMessage', JSON.stringify({
                ...
                $("#PINbox").val("");
                $.post('http://HD-wapenkluis/Click', JSON.stringify({}))
                $.post('http://HD-wapenkluis/EnterPincode', JSON.stringify({
                    pin: data.pin
                }))
