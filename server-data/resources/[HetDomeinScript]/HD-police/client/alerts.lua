...
AddEventHandler('HD-police:client:send:alert:gunshots', function(Coords, GunType, StreetName, InVeh)
   if (HDCore.Functions.GetPlayerData().job.name == "police") and HDCore.Functions.GetPlayerData().job.onduty then
    local AlertMessage, dienstnummer = 'Schoten gelost', 'CIC'
    if InVeh then
...
AddEventHandler('HD-police:client:send:banktruck:alert', function(Coords, Plate, StreetName)
 if (HDCore.Functions.GetPlayerData().job.name == "police") and HDCore.Functions.GetPlayerData().job.onduty then 
    ...
    AddAlert('Bank Truck Alarm', 67, 250, Coords, false)
 end
end)

RegisterNetEvent('HD-police:client:send:auto:alert')
AddEventHandler('HD-police:client:send:auto:alert', function(Coords, Plate, StreetName)
 if (HDCore.Functions.GetPlayerData().job.name == "police") and HDCore.Functions.GetPlayerData().job.onduty then 
    TriggerEvent('HD-alerts:client:send:alert', {
        timeOut = 15000,
        alertTitle = "Bank Truck Alarm",
    ...
    AddAlert('Voertuig Alarm', 67, 250, Coords, false)
 end
end)

RegisterNetEvent('HD-police:client:send:run:alert')
AddEventHandler('HD-police:client:send:run:alert', function(Coords, StreetName)
 if (HDCore.Functions.GetPlayerData().job.name == "police") and HDCore.Functions.GetPlayerData().job.onduty then 
    TriggerEvent('HD-alerts:client:send:alert', {
        timeOut = 15000,
...
function AddAlert(Text, Sprite, Transition, Coords, Tracker)
 ...
 if not Tracker then
  AddTextComponentString('NEW REPORT: '..Text)
 else
