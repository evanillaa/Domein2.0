...
RegisterNetEvent('HD-police:client:CameraCommand')
AddEventHandler('HD-police:client:CameraCommand', function(cameraId)
    local plyPed = PlayerPedId()
    local plyCoords = GetEntityCoords(plyPed)
    local distance = GetDistanceBetweenCoords(plyCoords.x, plyCoords.y,plyCoords.z,438.75601, -992.4405, 30.689327, true)

    --if distance < 4 then
        TriggerEvent('HD-police:client:ActiveCamera', cameraId)
    --end
end)

RegisterNetEvent('HD-police:client:ActiveCamera')
AddEventHandler('HD-police:client:ActiveCamera', function(cameraId)
    ...
    elseif cameraId == 0 then
        ...
        HDCore.Functions.Notify("Camera bestaat niet", "error")
    end
end)

RegisterNetEvent('HD-police:client:DisableAllCameras')
AddEventHandler('HD-police:client:DisableAllCameras', function()
    for k, v in pairs(Config.SecurityCameras.cameras) do 
        Config.SecurityCameras.cameras[k].isOnline = false
    end
end)

RegisterNetEvent('HD-police:client:EnableAllCameras')
AddEventHandler('HD-police:client:EnableAllCameras', function()
    for k, v in pairs(Config.SecurityCameras.cameras) do 
        Config.SecurityCameras.cameras[k].isOnline = true
    end
end)

RegisterNetEvent('HD-police:client:SetCamera')
AddEventHandler('HD-police:client:SetCamera', function(key, isOnline)
