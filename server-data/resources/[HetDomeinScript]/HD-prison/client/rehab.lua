...
AddEventHandler('beginJailRehab', function(isInRehabSent)
    ...
    HDCore.Functions.Notify("Welkom bij de afkickkliniek!", "success")
    Citizen.Wait(1000)
    ...
    if isInRehab then
        while isInRehab do
            ...
            if not inside then
                HDCore.Functions.Notify("De rechtbank heeft besloten dat je wat tijd in de afkickkliniek moet doorbrengen.", "success")
                SetEntityCoords(PlayerPedId(), groundCoords[placement][1],groundCoords[placement][2],groundCoords[placement][3]) 
            end
    ...
    HDCore.Functions.Notify("Je bent ontslagen uit de afkickkliniek.", "success")
