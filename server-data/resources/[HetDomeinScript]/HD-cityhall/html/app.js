...
$('.cityhall-option-block').click(function(e){
    ...
    if (blockPage == "identity") {
        ...
        $.post('http://HD-cityhall/requestLicenses', JSON.stringify({}), function(licenses){
    ...
    $.post('http://HD-cityhall/Click', JSON.stringify({}))
});
...
$(".request-identity-button").click(function(e){
    ...
    $.post('http://HD-cityhall/requestId', JSON.stringify({
...
$(document).on('click', '.apply-job-button', function(e){
    e.preventDefault();
    $.post('http://HD-cityhall/applyJob', JSON.stringify({
        job: selectedJobId
    }))
    Cityhall.ResetPages();
});
