$(document).on('click', '.spawn-dots', function(e){
    ...
    CloseContainer()
    $.post('http://HD-spawn/SpawnPlayer', JSON.stringify({
        SpawnId: SpawnDot
    }))
});
...
$(document).on('click', '.appartment-dots', function(e){
    ...
    CloseContainer()
    $.post('http://HD-spawn/ChooseAppartment', JSON.stringify({
        AppId: AppartmentDot
    }))
});
...
$(document).on('click', '.jail-dots', function(e){
    ...
    $.post('http://HD-spawn/SpawnJail', JSON.stringify({
        JailId: JailDot
    }))
});

CloseContainer = function() {
 $.post('http://HD-spawn/Close', JSON.stringify({})) 
...
OnClick = function() {
  $.post('http://HD-spawn/Click', JSON.stringify({}))  
}
